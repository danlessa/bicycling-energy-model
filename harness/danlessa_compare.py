...
extractRegimePowers, parseFIT/ptsFromFIT with the cadence field AND the
file_id manufacturer probe) are ported locally below, faithfully.
...
Reads data/activities/strava_danlessa_manifest.json (+ gitignored tracks);
writes results/danlessa_comparison.csv. Run: python3 harness/danlessa_compare.py
"""
...
# ---- FIT parsing — the .mjs's EXTENDED copy (cadence field + the file_id
# manufacturer probe that sets the FIT_MANUF global; 260 = Zwift). Structure
# mirrors bem.fit; out-of-bounds reads propagate (JS DataView throws
# RangeError → the ride counts as unparseable, exactly as in Node). ----
...
for lab, sub in [("all clean rides", eOK), ("s̄ ≥ 3%", [r for r in eOK if r["sbar"] >= 0.03])]:
    ...
    print("    frozen  clamp01(ε_coast − 0.13)      "
