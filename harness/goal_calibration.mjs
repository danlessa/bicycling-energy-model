...
function buildCache() {
  ...
  for (const corpus of CORPORA) {
    const man = JSON.parse(fs.readFileSync(path.join(DATA, `strava_${corpus}_manifest.json`), 'utf8'));
...
// per-ride CSV (gitignored via results/*)
{
  ...
  fs.writeFileSync(path.join(RESULTS, 'goal_calibration.csv'), lines.join('\n') + '\n');
