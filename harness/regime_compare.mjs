...
// compare.mjs + applet/index.html — the ppaz engine block is a substring, asserted at
...
// Output: console report + regime_comparison.csv (gitignored via results/*).
...
try {
  ...
  for (const e of inputs) {
    if (!e.file || !e.has_power || !fs.existsSync(path.join(DATA, e.file))) continue;
...
fs.writeFileSync(path.join(RESULTS, 'regime_comparison.csv'), csv + '\n');
