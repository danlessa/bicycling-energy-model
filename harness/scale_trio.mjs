...
const refCsv = parseCsv(fs.readFileSync(path.join(RESULTS, 'igc_resolution_test.csv'), 'utf8'));
...
function buildRiderSupp(corpus, ride) {
  ...
  const buf0 = fs.readFileSync(path.join(DATA, mr.file));
...
function processCenso(entry) {
  ...
  const buf0 = fs.readFileSync(path.join(DATA, entry.file));
...
fs.writeFileSync(path.join(RESULTS, 'scale_trio.csv'), run1.csv);
