...
these are the concrete thresholds, chosen before looking at any result):
  - OSM spans: Overpass `way["highway"]["bridge"]["bridge"!="no"]` +
    ...
    CACHED under results/e26_osm_cache/q2a/ keyed by a hash of the rounded tile bbox, so
...
Output: results/e26_portal_profiles.csv (gitignored via results/*) — per-ride, plus an
    ...
    for w in ways:
        ...
        for i0, i1 in runs_from_mask(mask, RUN_GAP_PTS):
            ...
            if extent < MIN_COVER * deck or extent > EXTENT_MAX_RATIO * deck:
                tally("extent-off")
                continue
            # plain floats — np.float64 would break js_num inside sample_raster
            if xA <= xB:
