...
function listRiders() {
  const R = [];
  const load = (mf, filt) => JSON.parse(fs.readFileSync(path.join(DATA, mf), 'utf8')).filter(filt);
...
fs.writeFileSync(path.join(RESULTS, 'param_fit.csv'), 'rider,mass_kg,cda_m2,crr,wind_ms_debiased,alpha,nAct,nGood\n' +
