...
the discretized optimum to Entry 19's fixed-route numbers)".
  ...
  E_opt(n)      free-terrain optimal energy A→B on the n-direction grid, from
                results/e26_grid.csv (produced in the sibling simujaules repo by
...
under the SAME bundle the grid harness uses, including the same gravity constant:
  ...
  * g = 9.81 — the constant the JS engine folds into its cost bundle.  This is
    DELIBERATELY not analysis/bem's G (São Paulo's 9.7864): a cross-engine ratio
...
Reads:  results/e26_grid.csv, results/e26_pair_rides.json (+ the gitignored FITs)
Writes: results/e26_detour.csv          (gitignored — carries ride labels)
stdout: aggregates only, no coordinates and no ride names.

  E26_GRID=<csv>  score a different grid CSV (e.g. e26_grid_cal.csv)
  /Users/danlessa/conda/bin/python harness/e26_detour.py
"""
...
def main():
    ...
    print(f"NOTE: g={G_JS} (the JS constant), deliberately NOT analysis/bem's G={igc.G} — "
