...
Reads data/activities/strava_jaam_manifest.json (+ gitignored tracks);
writes results/jaam_comparison.csv. Run: python3 harness/jaam_compare.py
"""
...
def jsdiv(a, b):
    """a / b with ECMAScript semantics (x/0 -> ±Infinity, 0/0 -> NaN)."""
    if b != 0:
        return a / b
    if a != a or a == 0:
        return float("nan")
    return float("inf") * math.copysign(1, a) * math.copysign(1, b)
...
# manufacturer probe FIT_MANUF; 260 = Zwift -> virtual ride). Structure
# mirrors bem.fit, plus this harness's own signature error ('no .FIT').
