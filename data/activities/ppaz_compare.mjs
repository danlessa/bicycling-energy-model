#!/usr/bin/env node
// SECOND-RIDER verification: P. Paz's Strava history export (strava_ppaz/, gitignored —
...
//
// Output: console report + ppaz_comparison.csv (gitignored via data/activities/*.csv).
...
// ===== driver =====
const man = JSON.parse(fs.readFileSync(path.join(HERE, 'strava_ppaz_manifest.json'), 'utf8'));
const CAND = man.filter(a => a.sport === 'ride' && a.powCov > 0.5 && a.km >= 20 && a.altCov >= 0.99);
console.log(`P. PAZ SECOND-RIDER VERIFICATION — ${CAND.length} candidate rides (ride, power>50%, ≥20 km, alt≥99%)`);
...
const M_USE = process.env.PPAZ_M ? +process.env.PPAZ_M : mHat;   // PPAZ_M env: mass-sensitivity runs
console.log(`  → using m = ${M_USE.toFixed(1)} kg ${process.env.PPAZ_M ? '(PPAZ_M override)' : '(per-ride median; robust to power dropouts)'}\n`);
...
console.log('\n================================================================');
console.log('ε SECOND-RIDER TEST — estimators FROZEN from rider 1 (nothing refit)');
for (const [lab, sub] of [['all clean rides', eOK], ['s̄ ≥ 3%', eOK.filter(r => r.sbar >= 0.03)]]) {
  ...
  console.log(`    in-sample flat = median ε_bal (${f(flatIn, 2)})  ${f(rms(eb.map(x => x - flatIn)), 3)}   <- P. Paz's own best constant`);
}
...
fs.writeFileSync(path.join(HERE, 'ppaz_comparison.csv'), csv + '\n');
console.log(`\nwrote ppaz_comparison.csv (${rows.length} rides)`);
