...
// compare.mjs's ptsFromGPX + energy-model-comparison.html's approxTime — keep in sync).
...
// Output: console report + time_comparison.csv (gitignored via data/activities/*.csv).
...
const csv = [cols.concat(['T1b_pred', 'T0_pred']).join(',')]
  ...
  .join('\n');
fs.writeFileSync(path.join(HERE, 'time_comparison.csv'), csv + '\n');
