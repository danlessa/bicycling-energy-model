...
function listRiders() {
  const R = [];
  try { const man = JSON.parse(fs.readFileSync(path.join(HERE, 'strava_ppaz_manifest.json'), 'utf8'));
    R.push({ name: 'P. Paz', files: man.filter(a => a.sport === 'ride' && a.powCov > 0.5 && a.km >= 20).map(a => a.file) }); } catch (e) { console.error('ppaz?', e.message); }
  try { const man = JSON.parse(fs.readFileSync(path.join(HERE, 'strava_jaam_manifest.json'), 'utf8'));
    R.push({ name: 'JAAM', files: man.filter(a => a.sport === 'ride' && a.powCov > 0.5 && a.km >= 20).map(a => a.file) }); } catch (e) { console.error('jaam?', e.message); }
  try { const inp = JSON.parse(fs.readFileSync(path.join(HERE, 'model_inputs.json'), 'utf8')).filter(e => e.has_power && e.file);
...
fs.writeFileSync(path.join(HERE, 'cda_estimate.csv'),
