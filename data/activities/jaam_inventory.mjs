#!/usr/bin/env node
// Inventory of JAAM's Strava history export (data/activities/strava_jaam/, gitignored:
...
// any coordinate), and writes strava_jaam_manifest.json (gitignored via data/activities/*.json).
//
...
const DIR = path.join(HERE, 'strava_jaam');
...
for (const f of files) {
  try {
    ...
    out.push({
      id: f.replace(/\.fit(\.gz)?$/, ''), file: path.join('strava_jaam', f),
...
fs.writeFileSync(path.join(HERE, 'strava_jaam_manifest.json'), JSON.stringify(out, null, 1));

// ---- summary ----
const bySport = {};
for (const a of out) (bySport[a.sport] ??= []).push(a);
console.log(`JAAM STRAVA EXPORT — ${files.length} FIT files, ${out.length} parsed, ${errors} errors\n`);
