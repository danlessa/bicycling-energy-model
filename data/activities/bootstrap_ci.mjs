...
//   censohidrografico/censo_comparison.csv    (censo_compare.mjs, 62 clean)
...
function parseCSV(p) {
  const text = fs.readFileSync(path.join(HERE, p), 'utf8').trim();
...
const cz = parseCSV('censohidrografico/censo_comparison.csv').filter(r => r.dataOK === 'true');
