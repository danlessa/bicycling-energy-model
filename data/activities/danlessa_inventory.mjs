#!/usr/bin/env node
// Inventory of JAAM's Strava history export (data/activities/strava_danlessa/, gitignored:
// third-party GPS shared with consent — an INDEPENDENT rider, not a Pedal Hidrográfico
// member). Multi-country (São Paulo + Colombia, Germany, Ukraine, US, …), spanning
// mountainous to plain terrain — the strongest external-validity corpus in the study.
//
// Goal: find CYCLING rides WITH POWER (the empirical ∫P·dt benchmark needs a power meter).
// Scans every .fit / .fit.gz, reads sport + record stats + non-locational terrain metrics
// (median altitude, raw ascent — characterise the geographic/terrain spread WITHOUT storing
// any coordinate), and writes strava_danlessa_manifest.json (gitignored via data/activities/*.json).
//
//   node jaam_inventory.mjs
//
// parseFIT is the verbatim copy used by all harnesses (ppaz_inventory.mjs) — the record
// decoding is untouched; keep in sync.
...
const DIR = path.join(HERE, 'strava_danlessa');
...
const median = xs => { const s = xs.filter(Number.isFinite).slice().sort((a, b) => a - b); const k = (s.length - 1) / 2; return s.length ? (s[Math.floor(k)] + s[Math.ceil(k)]) / 2 : null; };
...
for (const f of files) {
  try {
    ...
    const alts = []; let ascentRaw = 0, lastAlt;
    for (const r of recs) {
      if (r.power !== undefined) { nPow++; if (r.power > 0) nPowPos++; }
      if (r.cad !== undefined) nCad++;
      if (r.alt !== undefined) {
        nAlt++; alts.push(r.alt);
        if (lastAlt !== undefined && r.alt > lastAlt) ascentRaw += r.alt - lastAlt;
        lastAlt = r.alt;
      }
      if (r.dist !== undefined && r.dist > maxDist) maxDist = r.dist;
      if (r.time !== undefined) { if (t0 === undefined) t0 = r.time; t1 = r.time; }
    }
    const medAlt = median(alts);          // characterises altitude/geography (non-locational)
    out.push({
      id: f.replace(/\.fit(\.gz)?$/, ''), file: path.join('strava_danlessa', f),
      ...
      medAlt: medAlt == null ? null : +medAlt.toFixed(0),                            // m
      ascentRaw: +ascentRaw.toFixed(0),                                              // m (raw Σ+Δalt)
      ascentPerKm: maxDist > 500 ? +(ascentRaw / (maxDist / 1000)).toFixed(1) : null, // m/km terrain proxy
    });
  } catch (e) { errors++; }
}
fs.writeFileSync(path.join(HERE, 'strava_danlessa_manifest.json'), JSON.stringify(out, null, 1));

// ---- summary ----
const bySport = {};
for (const a of out) (bySport[a.sport] ??= []).push(a);
console.log(`AUTHOR (danlessa) STRAVA EXPORT — ${files.length} FIT files, ${out.length} parsed, ${errors} errors\n`);
...
if (rides.length) {
  ...
  const q = (arr, p) => { const s = arr.filter(Number.isFinite).slice().sort((a, b) => a - b); return s.length ? s[Math.floor(p * (s.length - 1))] : NaN; };
  const kms = rides.map(r => r.km), alts = rides.map(r => r.medAlt), apk = rides.map(r => r.ascentPerKm);
  console.log(`\nRIDES WITH POWER: ${rides.length}`);
  console.log(`  dates ${dates[0]} … ${dates[dates.length - 1]}`);
  console.log(`  km: min ${q(kms,0)}  p25 ${q(kms,.25)}  median ${q(kms,.5)}  p75 ${q(kms,.75)}  max ${q(kms,1)}`);
  console.log(`  median altitude (m): p10 ${q(alts,.1)}  median ${q(alts,.5)}  p90 ${q(alts,.9)}  max ${q(alts,1)}   [geographic/altitude spread]`);
  console.log(`  ascent/km (m/km): p10 ${q(apk,.1)}  median ${q(apk,.5)}  p90 ${q(apk,.9)}  max ${q(apk,1)}   [terrain: plain↔mountainous]`);
  console.log(`  rides above 1500 m median alt (high-altitude): ${rides.filter(r => r.medAlt >= 1500).length}`);
