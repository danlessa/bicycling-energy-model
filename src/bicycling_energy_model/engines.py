...
identical to the JS — the retired cross-language parity harness verified the
...
# local value is the physical one.  This constant is hand-kept-in-sync between
# the applet and this package (the harnesses import it from here) — a
# re-baseline that moves one and not the other silently mixes two gravities
...
def approx_components(prof: Profile, p: SimParams, vf: float,
    ...
    for i in range(1, len(xs)):
        ...
        slope = dh / dx
        X += dx
        aero_dx = 0.0 if slope >= climb_thr else a_aero   # cf: aero only off climbs
        aero_sum += aero_dx * dx
        if dh >= 0:
            hplus += dh
        else:
            hminus += -dh
    return {"roll": a_roll * X, "aero": aero_sum, "climb": beta * hplus,
