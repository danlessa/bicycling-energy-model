...
Python port of the retired scale_trio.mjs (same console report, byte-identical
data/results/scale_trio.csv).
...
IMPORTING those same functions from src/harness/regime_compare.py, src/harness/igc_resolution_test.py
and src/harness/goal_calibration.py — the one intentional structural deviation; everything numeric
...
# cache dir from the original run; regenerates anywhere if absent
...
# physics (for the E2 side-by-side; from Entry 20's run at σ=0). Re-baselined 2026-07-25
# (Entry 27): G = 9.7864 + the corrected implied masses (74.5/101.9/74.7) — values from the
# clean goal_calibration re-run; the FITS below refresh whenever Entry 20 re-runs.
E20_SIGMA0_UNCAL_VAL = {"ppaz": 8.535, "jaam": 2.636, "danlessa": 14.862}
...
def process_censo(entry):
    ...
    aSm0 = RC.approx_components(profS0, p, flat_eq_speed(overall_mean_power(pts), p),
                                CLIMB_THR)
