...
Python port of the retired goal_calibration.mjs (same console report, byte-identical
data/results/goal_calibration.csv).
...
Pre-registered protocol (journal Entry 20): two deployable levers only —
  ...
  L2 (per-rider = the app's parameter panel): (CdA ∈ [0.2,0.6], Crr ∈ [0.003,0.015],
      kSmooth ∈ [0.5,1.0]) fitted per rider on TRAIN; mass FROZEN (74.5/101.9/74.7),
...
RASTER SMOOTHING (Phase A) is src/harness/goal_smooth_rasters.py — unchanged, still run with
...
functions from src/harness/regime_compare.py and src/harness/igc_resolution_test.py — the one
...
(data/results/igc_resolution_test.csv, counts asserted 277/181/406); every ride is INDEPENDENTLY
...
def build_ride_profiles(corpus, row, file):
    ...
    if abs(emp - row["emp"]) > 1e-3:
        raise ValueError(f"emp mismatch {corpus}/{row['ride']}: "
                         f"{js_str(emp)} vs csv {js_str(row['emp'])}")
    ...
    for sig in SIGMAS:
        ...
        if not b["prof"] or b["validFrac"] < 0.99:
            raise ValueError(f"coverage {corpus}/{row['ride']} sigma={sig}: "
                             f"validFrac={js_str(b['validFrac']) if b else 'null'}")
        hs.append(b["prof"]["h"])
...
def main():
    ...
    # per-ride CSV (gitignored via data/results/*)
    si = run1["sigStarIdx"]
