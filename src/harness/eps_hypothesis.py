...
Reads data/inputs/activities/model_inputs.json + eps_features.json (+ the gitignored
...
from bicycling_energy_model import finish_pts, parse_fit
...
# the retired analysis/parity/reference.mjs — kept in git history
# (so they are ported here, not imported):
# haversine (same arithmetic), ptsFromFIT
...
def haversine(a: dict, b: dict) -> float:
    ...
    s = s1 * s1 + math.cos(a["lat"] * t) * math.cos(b["lat"] * t) * (s2 * s2)
...
csv_rows = [",".join([
    json.dumps(r["ride"], ensure_ascii=False), js_str(r["epsBal"]), js_str(r["epsCoast"]),
    js_str(r["epsLump"]), js_str(r["sbar"]), js_str(r["bHd"]),
    "" if r["kappa"] is None else js_str(r["kappa"]),
    "" if r["unpaved"] is None else js_str(r["unpaved"]),
    "" if r["sheet"] is None else js_str(r["sheet"]),
]) for r in good]
