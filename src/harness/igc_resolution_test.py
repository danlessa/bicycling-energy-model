...
Python port of the retired igc_resolution_test.mjs (same console report, byte-identical
data/results/igc_resolution_test.csv).
...
IMPORTING those same functions from src/harness/regime_compare.py — the one intentional structural
...
(0.000287042610744° = 6 × the native 0.000047840435124° pixel ≈ 30 m at this latitude.)
  ...
  python3 src/harness/igc_resolution_test.py   → report on stdout (timings on stderr) +
                                                 igc_resolution_test.csv (gitignored via data/results/*)
...
from regime_compare import (ASSUMED, CLIMB_THR, DESC_THR, ENGINE_DX, G,  # noqa: E402,F401
                            ...
                            bin_grades, build_profile, d_pct,
                            deadband, empirical_kj, eps_geom, flat_eq_speed,
                            has_power, haversine, jge, jgt, jquote,
...
def sample_raster(raster, lons, lats, timer_key):
    ...
    for i in range(len(lons)):
        parts.append(js_str(lons[i]) + ' ' + js_str(lats[i]) + '\n')
...
def process_ride(file, p0, label, corpus, eps_rule):
    ...
    if corpus == 'censo':   # physical-plausibility floor, VERBATIM regime_compare censo driver
        ...
        aSm0 = approx_components(profS0, p, flat_eq_speed(overall_mean_power(pts), p),
                                 CLIMB_THR)
