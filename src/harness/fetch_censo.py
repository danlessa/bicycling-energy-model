...
(columns Q 'Ativ. Strava' / R 'Ativ. RWGPS') into data/inputs/activities/censohidrografico/.
...
DATA = os.path.join(os.path.dirname(os.path.dirname(HERE)), "data", "inputs", "activities")
DEST = os.path.join(DATA, "censohidrografico")
    ...
    def run(e):
        if e["source"] == "rwgps":
            ...
            if os.path.exists(dest):
                e["file"] = os.path.relpath(dest, DATA); return (e, "skip", None)
            ...
            e["file"] = os.path.relpath(dest, DATA); return (e, "ok", None)
        else:  # strava
            if not jar:
            ...
            if final_existing:
                e["file"] = os.path.relpath(final_existing, DATA); return (e, "skip", None)
