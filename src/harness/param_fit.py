...
Python port of the retired param_fit.mjs (same stdout, byte-identical CSV).
...
CdA·Wn) per activity, iterated 3× to fold the w² term back in.
  ...
  python3 src/harness/param_fit.py     -> data/results/param_fit.csv + the console report

WHY THIS ONE STILL CARRIES SOME OF ITS OWN MATH (and not just src/bicycling_energy_model):

* ``pts_with_geo`` is the one point-builder in the repo that is NOT the
  verbatim pts_from_fit — it keeps lat/lon because the fit needs a per-sample
  BEARING (see CLAUDE.md).  It stays local, along with the haversine it needs.
...
from bicycling_energy_model import is_finite, jsdiv, parse_fit
...
def pts_with_geo(recs):
    ...
    for i in range(n):
        ...
        b = pts[min(n - 1, i + 1)]
        d_lon = (b["lon"] - a["lon"]) * TO_R
        cos_blat = math.cos(b["lat"] * TO_R)                # used twice; same value
        y = math.sin(d_lon) * cos_blat
        xb = (math.cos(a["lat"] * TO_R) * math.sin(b["lat"] * TO_R)
              - math.sin(a["lat"] * TO_R) * cos_blat * math.cos(d_lon))
        pts[i]["bear"] = math.atan2(y, xb)
        j = i
...
def fit_activity(pts, m):
    ...
    for p in pts:
        ...
        S.append({"v": v, "pw": p["power"], "acc": p["acc"], "sin": grade / sec,
                  ...
                  "sinB": math.sin(bear), "cosB": math.cos(bear), "v3": math.pow(v, 3)})
    if len(S) < 200:
        return None
    n_aero = 0
    for s in S:
    ...
    return {"cda": cda, "crr": crr, "We": We, "Wn": Wn, "W": js_hypot(We, Wn),
            "windDir": math.fmod(math.atan2(We, Wn) / TO_R + 360, 360),
...
if os.environ.get("SYNTH"):
    ...
    print(f"SYNTHETIC WIND RECOVERY — inject (We={js_str(We0)}, Wn={js_str(Wn0)}) m/s, "
    ...
    for f in R0["files"]:
        ...
        for p in pts:
            ...
            sin = p["grade"] / sec
            cos = 1 / sec
            w = -(We0 * math.sin(p["bear"]) + Wn0 * math.cos(p["bear"]))
            air = p["v"] + w
...
for r in list_riders():
    ...
    print(f"  MASS  = {to_fixed(m, 1)} kg   [rider-level, {rm['nSeg']} climb seg]      "
          f"target {js_str(rng['m'][0])}–{js_str(rng['m'][1])}  {in_r(m, rng['m'])}")
    print(f"  CdA   = {to_fixed(median(cdas), 3)} m²  [IQR {to_fixed(q(cdas, .25), 2)}–"
          f"{to_fixed(q(cdas, .75), 2)}]   target {js_str(rng['cda'][0])}–"
          f"{js_str(rng['cda'][1])}  {in_r(median(cdas), rng['cda'])}")
    print(f"  C_rr  = {to_fixed(median(crrs), 4)}   [IQR {to_fixed(q(crrs, .25), 4)}–"
          f"{to_fixed(q(crrs, .75), 4)}]  target {js_str(rng['crr'][0])}–"
          f"{js_str(rng['crr'][1])}  {in_r(median(crrs), rng['crr'])}")
