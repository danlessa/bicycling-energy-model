...
OUT  = os.path.join(REPO, "data", "inputs", "activities")
...
def main():
    ...
    for r in rows:
        ...
        if not path:
            continue
        # longoes.csv stores legacy repo-root-relative paths (data/activities/…);
        # remap the moved prefix onto the new layout (data/inputs/activities/ == OUT).
        if path.startswith("data/activities/"):
            ap = os.path.join(OUT, path[len("data/activities/"):])
        else:
    ...
    print(f"\nfull detail -> data/results/longoes_verify.csv ({len(detail)} comparisons)")
