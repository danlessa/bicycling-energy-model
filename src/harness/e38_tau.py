#!/usr/bin/env python3
"""Entry 38 — the τ-sweep: does the optimal deadband track h_KE = v_f²/2g?

Pre-registered in MODEL_COMPARISON_JOURNAL.md Entry 38 BEFORE any result was
seen. Per corpus (D1–D5, Entry-33 populations), form 3 (split + deadband at
τ) is evaluated over τ ∈ {0.5 … 6.0 m, step 0.5} under the frozen
shared-constants protocol (anchor/logged mass, Crr 0.008, CdA 0.40, ρ 1.13,
k_eff 0.98, wind 0) — Table 3's protocol, so the τ = 2.0 column must
reproduce the published medians (built-in parity gate). ε_d comes from
eps_geom on the RAW profile (τ-independent by construction); ε_f = 0.20.
τ* = argmin med|Δ%| of the corpus's regime-appropriate variant (ε_d
everywhere except D2's ε_f), with a B = 1,000 bootstrap CI on τ* (disclosed
deviation from the 10⁴ convention: stdlib runtime). h_KE targets from the
corpus median measured flat speed.

Env: E38_SMOKE=1 (40 rides/corpus).
Run: python3 src/harness/e38_tau.py
Output: data/results/e38_tau.csv + console tables.
"""
...
SMOKE = os.environ.get("E38_SMOKE") == "1"
...
PRIMARY = {"longoes": "d", "censo": "f", "ppaz": "d", "jaam": "d",
           "danlessa": "d"}
PUBLISHED_T20 = {"censo": ("f", 4.7), "ppaz": ("d", 5.8), "jaam": ("d", 5.5),
                 "danlessa": ("d", 6.2)}       # Table 3 parity check at τ=2
...
def main() -> None:
    ...
    for corpus in ("longoes", "censo", "ppaz", "jaam", "danlessa"):
        ...
        if corpus in PUBLISHED_T20:
            ...
            ok = abs(got - pub) <= 0.11
            print(f"PARITY τ=2.0 ε_{ptag} vs Table 3: {to_fixed(got, 2)} vs {pub} "
                  + ("GATE-OK" if ok else "GATE-FAIL"))

    cols = list(all_rows[0].keys())
    with open(os.path.join(RESULTS, "e38_tau.csv"), "w", encoding="utf-8") as fh:
        fh.write(",".join(cols) + "\n")
        for r in all_rows:
            fh.write(",".join((f'"{v}"' if isinstance(v, str) else to_fixed(v, 4))
                              for v in (r[k] for k in cols)) + "\n")
    print(f"\nwrote e38_tau.csv ({len(all_rows)} rides)")
