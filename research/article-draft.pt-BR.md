...
# Um Fator de Recuperação na Descida em Forma Fechada Validado para a Energia de Rotas de Bicicleta, e Seu Dual Energia↔Tempo

> **RASCUNHO / artigo em elaboração — notas de pesquisa do Pedal Hidrográfico.** Os números e as equações provêm dos próprios benchmarks do projeto; as alegações de novidade são limitadas pelo corpus (ver §11.3). Não revisado por pares; circulado para revisão e comentários internos.

**Danilo Lessa Bernardineli** — *Pedal Hidrográfico* (coletivo), São Paulo, Brazil — danilo.lessa@gmail.com

## Resumo

O planejamento de pedaladas comunitárias de bicicleta precisa da *energia* de uma rota em kJ, mas a ferramenta padrão é uma simulação de dinâmica direta [Martin et al. 1998] que é cara e opaca para o planejamento. Estudamos uma alternativa barata em forma fechada, `E ≈ α·x + β·(h₊ − ε·h₋)`, na qual um único fator agregado `ε ∈ [0,1]` captura quanto da energia potencial da descida é recuperada em vez de desperdiçada em arrasto excedente e frenagem. Damos a `ε` uma forma fechada no limite de inércia, `ε(s) = min(1, α/(β·s))`, com um agregado ponderado pela queda e um offset calibrado de `−0.13`, e a validamos contra um `ε` de balanço de energia na descida medido por medidor de potência em descidas reais (correlação subindo a 0.83 com inclinação média ≥ 3% e 0.87 com ≥ 3.5%). Rodada nas *mesmas* constantes físicas da simulação, a forma fechada reproduz o `∫P·dt` medido com uma mediana de 3.4% sobre 44 pedaladas com medidor de potência (melhor variante) e com ~4–7% sobre 62 pedaladas urbanas em São Paulo com um ciclista genérico assumido. Como a subida acumulada é fractal e depende da escala [Rapaport 2011], incorporamos uma correção apenas de totais `k_smooth = 1 − c·x/h₊` (`c ≈ 3 m/km`) à lei em forma fechada para descontar o ruído submétrico sem afetar as subidas sustentadas. Derivamos ainda um dual energia↔tempo, `x* = x + k₊·h₊ − k₋·h₋`, no qual o coeficiente de descida `k₋` é o gêmeo temporal de `ε` através da mesma potência na descida; ambas as metades do modelo de tempo têm precedente individualmente, mas não localizamos nenhum trabalho anterior derivando o crédito de tempo na descida a partir da mesma potência na descida que o fator de recuperação de energia — o *vínculo* é a peça inédita. Ambos os motores e a lei compartilhada estão implantados em ferramentas abertas e local-first (sampasimu, amora, quilojaules).
...
- **Um fator de recuperação na descida `ε` em forma fechada, em nível de rota e validado.** Um único `ε ∈ [0,1]` agregado dentro de `E ≈ α·x + β·(h₊ − ε·h₋)`, com sua forma fechada no limite de inércia `ε(s) = min(1, α/(β·s))`, agregado ponderado pela queda e offset calibrado de `−0.13`. Nenhum precedente para tal `ε` agregado em forma fechada foi localizado no corpus mais próximo de potência no ciclismo, roteamento por elevação ou energia de VE/e-bike.
- **Validação contra um `ε` de balanço de energia na descida medido por potência** em descidas reais: correlação 0.38 (todas as 44 pedaladas) → 0.65 (ponderado pela energia de descida) → 0.83 (inclinação média ≥ 3%, n = 22) → 0.87 (≥ 3.5%, n = 15). Exemplo resolvido (RMC200 Mogi): `α/β = 0.0202`, `s̄ = 3.4%` ⇒ `ε = 0.46` previsto vs `0.47` medido.
- **Uma dualidade energia↔tempo** `x* = x + k₊·h₊ − k₋·h₋` cujo coeficiente de descida `k₋` é o gêmeo temporal de `ε`, tornados inter-deriváveis através da potência na descida compartilhada `P̄_desc`. Ambas as metades do modelo de tempo têm arte anterior individualmente; a *derivação de `k₋` a partir da mesma potência na descida que `ε`* é, até onde sabemos, inédita.
...
- **Validação em pedaladas urbanas e passeios coletivos reais e não competitivos** reproduzindo o `∫P·dt` medido com uma mediana de 3.4% (melhor variante em forma fechada) sobre 44 pedaladas com medidor de potência e com mediana de ~4–7% sobre 62 pedaladas urbanas em São Paulo, com filtros explícitos de piso físico (`E_legs ≥ m·g·h₊/k_eff`) e de qualidade de dados de cadência — e um resultado negativo documentado de São Paulo (a pedalada urbana de para-e-anda faz `ε` comportar-se como uma constante aproximada em vez de acompanhar a densidade de frenagem).
...
integrado com uma atualização semi-implícita da energia cinética, um limite de frenagem/velocidade segura e uma identidade de conservação imposta `k_eff·legE = ΔKE + W_rr + W_aero + W_grav + W_brake`. Não reivindicamos nada de novo sobre a física aqui. O que acrescentamos é operacional: uma implementação de referência limpa, aberta e sem etapa de build, e seu uso como o braço de *controle* de uma comparação de constantes compartilhadas (§4). Somos escrupulosos em um ponto: [Martin et al. 1998] fazem simplificações de pequeno ângulo *dentro* desse modelo de potência instantânea, mas nunca publicam uma forma fechada de energia em nível de rota; a forma fechada de §4–5 é nossa própria derivação, não autorizada por Martin.
...
O que é, sem precedente localizado na literatura mais próxima de potência de ciclismo de estrada e de roteamento por elevação, é o **fator de recuperação na descida agregado, em nível de rota, ε ∈ [0,1]** e sua forma fechada. Escrevemos
...
O ε ∈ [0,1] agregado, em nível de rota, em forma fechada, é a contribuição sem precedente localizado na arte prévia mais próxima. O roteamento de VE/e-bike e de pesquisa operacional sempre modela a recuperação na descida como uma eficiência de regeneração por instante ou por faixa de velocidade, um potencial simétrico separado $(M+m)g\Delta H$ [Ahmadi et al. 2026], ou custos de arco negativos por aresta resolvidos numericamente [Perger & Auer 2020]. O parente conceitual mais próximo para o limite ocioso/de inércia de ε é a fronteira de potência de tração de inclinação negativa de [Bigazzi & Lindsey 2019] ($v^2 \le \mu_1/(-\mu_3)$ ⇒ potência de tração 0) — mas eles a aplicam à escolha de velocidade em regime permanente por inclinação, nunca a uma forma fechada em nível de rota.
...
Esta é a estimativa de planejamento apenas geométrica `epsGeom()`: ela não precisa de registro de potência, apenas do perfil de inclinação da rota e das constantes do ciclista. Testamo-la, derivamos o deslocamento de `−0.13` e validamos o estimador resultante na §8.3; o resultado principal é correlação 0.83–0.87 contra um ε medido por potência em descidas reais, calibrando para
...
de modo que $k_-$ é um **parâmetro agregado e ajustado** — desempenhando para o tempo exatamente o papel que ε desempenha para a energia. A correspondência termo a termo é o coração estrutural da dualidade:
...
O que não tem precedente localizado é o **vínculo**. ε (um ajuste de energia) e k₋ (um ajuste de tempo) não são deriváveis um do outro isoladamente, mas tornam-se assim através da **potência na descida** $\bar P_{desc}$ — sendo a potência a taxa de câmbio entre energia e tempo. Ambos codificam a mesma velocidade de descida oculta $v_{desc}$.
...
e portanto, dados $\bar P_{desc}$ e a inclinação s, cada fator de ajuste em termos do outro:
...
Ambas as metades de x* têm precedente — a metade de subida na distância plana equivalente do Naismith ciclístico [Scarf & Grehan 2005; Scarf 2007; Norman 2004], a metade de descida nos créditos de tempo de caminhada [Langmuir 1984; Tobler 1993], todos ajustes empíricos nunca atrelados a um orçamento de energia. A contribuição genuinamente aditiva aqui é a **própria dualidade**: nenhum trabalho anterior deriva o crédito de tempo de descida k₋ a partir da *mesma potência na descida* $\bar P_{desc}$ que fixa o fator de recuperação ε.
...
**(ii) O escalar do pobre — apenas totais.** Quando só os totais da rota (`x`, `h₊`) estão disponíveis, a estimativa mais barata explora o fato de que a subida espúria se acumula com a *distância*, não com o terreno — uma "inclinação de ruído" aproximadamente constante que o MDE ou a trilha adiciona:
...
### 7.2 Os três conjuntos de dados
...
O mecanismo é diagnosticado, não assumido, via uma **verificação cruzada de cadência** que distingue uma queda do canal de potência de um ciclista de fato empurrando a bicicleta a pé (o que legitimamente teria `∫P·dt` abaixo do piso de PE). Para **6 das 7** pedaladas excluídas, a cobertura de cadência é de **73–100 %** enquanto o sinal de caminhada (movendo-se a < 4 km/h com cadência 0) é de apenas **~1 %** — ou seja, o ciclista *estava* pedalando e o déficit é um problema do sensor de potência, não caminhada. Apenas a 7ª pedalada (Mirantes, **31 % de cobertura de cadência**) é uma queda mais completa do sensor. Este filtro é conservador: descarta pedaladas cuja energia não pode ser confiável como referência, sem inflar silenciosamente a acurácia do modelo nas pedaladas que permanecem.
...
| esperado (gravidade 37 366 + rolamento 4 424 + aero 1 424) | 43 214 |
...
|---|--:|--:|
| suavizado (`cf` + banda morta real de 2 m, `k_smooth = 1`) | **3.4** | +2.2 |
| canônico (sim. direta) | 5.1 | −1.7 |
| `k_smooth` escalar (`cf` + `1 − c·x/h₊`, sem suavização) | 5.8 | −0.7 |

O escalar é essencialmente não enviesado (−0,7 %) mas carrega aproximadamente o dobro da dispersão da banda morta explícita.
...
|---|--:|--:|
| todas as 44 pedaladas (sem ponderação) | 0.38 | −0.18 |
| ponderado pela energia de descida `β·H₋` | **0.65** | −0.19 |
| descidas reais, s̄ ≥ 3.0 % (n = 22) | **0.83** | −0.13 |
| descidas reais, s̄ ≥ 3.5 % (n = 15) | **0.87** | −0.13 |

**A lei de inclinação vale precisamente onde ε carrega energia.** A correlação sobe de 0,38 sobre todas as pedaladas para 0,65 uma vez ponderada pela energia de descida `β·H₋`, e para 0,83 / 0,87 restringindo a descidas reais, percorríveis por inércia (s̄ ≥ 3,0 % / 3,5 %). A baixa correlação sobre todas as pedaladas é inofensiva: pedaladas suaves carregam `β·H₋ ≈ 0` de energia de descida, então um ε mal previsto nelas custa quase nada em kJ — que é exatamente por que a ponderação por energia eleva a correlação de 0,38 para 0,65.

Ao longo das pedaladas onde ε importa, o resíduo é um deslocamento quase **constante de −0.13** — a pedalada residual de descida e a frenagem que o ideal de pura inércia omite. (Isto é distinto dos viéses sem ponderação de −0,18 e ponderado por energia de −0,19 na tabela acima; o −0,13 é o deslocamento nas linhas de descida real que o estimador adota.) Subtraí-lo dá o estimador de trabalho `ε ≈ clamp_[0,1](ε_coast − 0.13)`, que transforma a mediana de ε_coast de 0,39 para s̄ ≥ 3 % em 0,26, igualando o medido 0,27, e supera a constante plana 0,23 / 0,27 da planilha. Um *exemplo resolvido* (RMC200 Mogi): `α/β = 0.0202`, `s̄ = 3.4 %` ⇒ `min(1, 0.0202/0.0341) = 0.59`; menos 0,13 ⇒ **0.46**, contra um medido **0.47**.
...
| Banda morta `k_smooth` para subida fractal *dentro de uma lei em forma fechada* | **Formalização nova** (aditiva) | [Rapaport 2011] (qualitativo, apenas em palavras) |
...
As limitações francas são três.

**Dispersão de ε guiada por comportamento e um resultado negativo em São Paulo.** ε não é uma propriedade só do terreno; depende de como o ciclista desce. O clamp-em-1 do limite de inércia se inverte em terreno suave — os ciclistas pedalam *através* das depressões, então o ε medido → 0 em vez de 1 (NS3 Caracaí: previsto ≈ 0.9, medido 0.01) — embora isso seja inofensivo porque tais pedaladas carregam `β·H₋ ≈ 0` de energia de descida, razão pela qual a ponderação pela energia eleva a correlação de todas as pedaladas de 0.38 para 0.65 (e a própria lei de inclinação atinge corr 0.83 em `s̄ ≥ 3%`, 0.87 em `s̄ ≥ 3.5%`). No corpus do censo urbano testamos a hipótese natural de que o crédito a mais residual de ε é guiado por frenagem e **a refutamos nos nossos próprios dados**: ao longo de 59 pedaladas limpas do censo, o gap (ε_coast − ε_true, medianas 0.35 e 0.14) não acompanha a densidade de para-e-anda — Δε_brake corr 0.08, paradas/km corr −0.28, todos R² ≤ 0.08 — e a correção mecanística corrige demais (Δε_brake mediana 0.34 ≫ gap 0.17), dando RMS 0.15, pior que uma constante plana. O mecanismo é que numa descida a gravidade, não as pernas, paga a re-aceleração pós-parada, então o sinal de frenagem não sobrevive até ε. A conclusão prática é desfavorável à forma fechada em terreno de para-e-anda: `ε_geom` (mediana 0.29) credita a mais, e um `ε ≈ 0.20` plano é o melhor estimador urbano (ótimo da varredura; `ε_coast − 0.13` RMS 0.12 vs plano-0.20 RMS 0.10). ε é, portanto, limitado ao corpus: use `ε_geom` em rotas abertas e inerciáveis, `ε ≈ 0.20` plano em para-e-anda urbano.
...
- **[Minetti et al. 2002]** Minetti, A. E. et al. (2002). *Energy cost of walking and running at extreme uphill and downhill slopes.* J. Appl. Physiol. 93(3):1039–1046. (Citado apenas como analogia conceitual de assimetria.)
