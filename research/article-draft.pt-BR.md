...
> **RASCUNHO / artigo em elaboração — notas de pesquisa do Pedal Hidrográfico** (v0.11, julho de 2026). Os números e as equações provêm dos próprios benchmarks do projeto; as alegações de novidade são limitadas pelo corpus (ver §11.3). Os números de acurácia são autorrelatados contra os medidores de potência de dois membros do coletivo (ver §10.4); a calibração de ε é intra-amostra no ciclista 1 e confirmada congelada no ciclista 2 (§8.6); as correlações de ε relatadas são parte-todo (§8.3); o modelo de tempo está meio confirmado (subida transfere-se, ponte de descida não — §8.8). Não revisado por pares; circulado para revisão e comentários internos.
