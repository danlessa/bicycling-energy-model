...
- **Avaliação contra um `ε` de balanço de energia na descida medido por potência**: uma redução de 37% no RMS sobre a melhor constante fixa em descidas reais (s̄ ≥ 3%, n = 22; intra-amostra, §8.3) — e, congelado, o mesmo estimador supera a melhor constante fixa de um **segundo ciclista** em ~35% sobre 156 pedaladas de descida real que nunca viu, com o offset de −0,13 reproduzindo-se independentemente (gap medido de 0,12; §8.6).
...
### 7.2 Os quatro conjuntos de dados
...
**Conjunto de dados 4 — Segundo ciclista (441 pedaladas com potência, P. Paz).** Um segundo membro do coletivo — um ciclista mais rápido, de estrada aberta — compartilhou seu histórico completo de exportação do Strava (2023-10 → 2026-07; compartilhado com consentimento, mantido localmente, nunca publicado). De 1 054 atividades FIT, **753 pedaladas carregam potência**; o harness mantém pedaladas ≥ 20 km com cobertura de altitude ≥ 99%, exclui 45 pedaladas virtuais (Zwift) via o fabricante do `file_id` FIT, e chega a **441 pedaladas utilizáveis** — nenhuma excluída pelo piso físico. A física do ciclista é assumida como no Conjunto 2 (CdA 0,40, C_rr 0,008, ρ 1,13) **exceto a massa total, que é invertida do próprio balanço de energia de subida sustentada do ciclista** (a maquinaria da §8.2): sobre 10 124 seções sustentadas (209 km de Δh), a massa implicada mediana por pedalada é **m̂ = 74,3 kg** [IQR 69,0–78,2]. Este conjunto é o **teste de transferência entre ciclistas** do artigo: cada constante calibrada — o offset de ε de −0,13, a inclinação de ruído c = 3 m/km — é congelada dos Conjuntos 1–2, e o ciclista, o medidor de potência e o perfil de pedalada (v_f mediano de 26,6 km/h vs 16,5 urbano) são todos novos. Resultados na §8.6.
...
Três coisas se destacam. Primeiro, o estimador geométrico calibrado **supera até a melhor constante fixa do próprio segundo ciclista em ~35%** (RMS 0,091 vs 0,139) num subconjunto de descidas reais sete vezes maior (n = 156) do que aquele em que foi calibrado (n = 22). Segundo, ao contrário do ciclista 1, ele também vence sobre *todas* as pedaladas (0,280 vs 0,356) — as pedaladas suaves deste ciclista ainda são inerciáveis, então a reversão de terreno suave do clamp (§8.3) quase não morde. Terceiro, **o offset de −0,13 reproduz-se independentemente**: o gap medido do ciclista 2, med(ε_coast) − med(ε_bal) em descidas reais, é **0,12** (0,48 − 0,36). Dois ciclistas, dois medidores de potência, o mesmo offset quase constante.

O resultado é insensível à única calibração intra-amostra em que se apoia: re-rodar com massa total 70/74,3/78 kg move o RMS do estimador congelado apenas 0,096/0,091/0,088 (e a referência fixa intra-amostra 0,147/0,139/0,133) — a margem de ~35% mantém-se do início ao fim. A correlação corr(ε_coast, ε_bal) = 0,81 em s̄ ≥ 3% carrega a mesma ressalva parte-todo da §8.3; a comparação de RMS congelado-vs-fixo é a estatística honesta, e ela é genuinamente fora da amostra. Ressalvas remanescentes: mesmo coletivo e mesma região da cidade (estradas e cultura de pedalada compartilhadas), CdA/C_rr do ciclista 2 ainda assumidos, e a avaliação compartilha seu *método* (células de 30 m, velocidade plana medida) com o ciclista 1 — um artefato de método não seria pego. Registro completo: entrada 12 do journal.

![Teste de ε do segundo ciclista](figs/fig6-ppaz-eps.svg)

*Figura 6. O teste do segundo ciclista: `ε_coast` apenas-geométrico vs `ε_bal` medido por potência para as 436 pedaladas de P. Paz, com a linha de calibração `ε = ε_coast − 0.13` congelada do ciclista 1 (verde). Em descidas reais (s̄ ≥ 3%, vermelhão, n = 156) a linha congelada acompanha as medições de um ciclista independente; área do ponto ∝ energia de descida.*
...
**Dois ciclistas, dois medidores de potência — mesmo coletivo.** As referências de energia vêm dos dispositivos próprios de dois ciclistas: o ciclista 1 (Conjuntos 1–2, 106 pedaladas) e o ciclista 2 (Conjunto 4, 441 pedaladas), cujos dados chegaram depois de as calibrações serem congeladas e em quem o estimador de ε foi confirmado fora da amostra (§8.6). A validade externa além de n = 2 permanece não testada, e ambos os ciclistas pertencem ao mesmo coletivo na mesma região da cidade — estradas e cultura de pedalada compartilhadas — então a transferência não sonda climas, regimes de trânsito ou categorias de bicicleta diferentes. O CdA/C_rr do ciclista 2 são assumidos (só a massa é implicada pelos dados, com o resultado de ε mostrado insensível a ela), e a avaliação de ε compartilha seu método (células de 30 m, velocidade plana medida) entre os ciclistas, então um artefato de método não seria pego.

**As correlações de ε são intra-amostra e parte-todo.** O deslocamento de −0,13 é calibrado nas mesmas 44 pedaladas sobre as quais as correlações são relatadas, e o preditor `ε_coast` compartilha seu termo dominante *e* seu α por pedalada com a "verdade" `ε_bal`, então os 0,77/0,82 relatados são inflacionados por construção — o termo geométrico compartilhado `α/(β·s̄)` sozinho correlaciona-se em 0,72 com `ε_bal` e em 0,99 com o próprio `ε_coast` (§8.3). Leia-os como um sinal de habilidade intra-amostra, não validação fora da amostra; a cifra de redução de RMS (37% em s̄≥3%) é a estatística mais defensável. O conjunto do censo fornece a checagem entre regimes, com veredito dividido: o offset de −0,13 congelado empata com a constante ótima intra-amostra no alvo de balanço de descida (§8.5), então a *calibração* transfere-se — mas o `ε_geom` geométrico completo tem desempenho inferior a um ε fixo = 0,20 na varredura de energia (§8.4), então o comportamento do clamp em terreno suave não. O segundo ciclista (§8.6) fornece então a checagem entre ciclistas, e lá o veredito é inequívoco: o estimador congelado supera até a melhor constante fixa do próprio ciclista em ~35% em descidas reais, e o offset reproduz-se independentemente (0,12 vs o 0,13 calibrado).
...
As gravações de potência e GPS analisadas aqui vêm de dois membros do coletivo: o autor (Conjuntos 1–2, no dispositivo do próprio autor, inclusive em pedaladas em grupo) e um segundo membro que compartilhou sua exportação completa de histórico de atividades **com consentimento informado** para esta análise (Conjunto 4; mantida localmente, nunca publicada, referida apenas pelas iniciais). Nenhum dado de outro ciclista é usado — atividades do censo gravadas por terceiros (16 links) foram excluídas em vez de acessadas. Os agregados reportados para o ciclista 2 (energias, ε, massa implicada) não carregam informação de localização. O histórico do repositório foi auditado quanto a vazamento de dados pessoais; todas as trilhas brutas, exportações e planilhas são excluídas do controle de versão.
