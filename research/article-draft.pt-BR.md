...
- **Validação contra um `ε` de balanço de energia na descida medido por potência** em descidas reais: correlação 0,30 (todas as 44 pedaladas) → 0,60 (ponderado pela energia de descida) → 0,77 (inclinação média ≥ 3%, n = 22) → 0,82 (≥ 3,5%, n = 15) — lida com a ressalva parte-todo da §8.3 (o ganho honesto é uma redução de 37% no RMS sobre uma constante fixa). Exemplo resolvido (RMC200 Mogi): `α/β = 0.0202`, `s̄ = 3.4%` ⇒ `ε = 0.46` previsto vs `0.47` medido.
...
de modo que $k_-$ é um **parâmetro agregado e livre** — desempenhando para o tempo exatamente o papel que ε desempenha para a energia (livre, não ajustado: nenhum tempo de pedalada medido o calibra em qualquer parte deste trabalho — ver §10.4). A correspondência termo a termo é o coração estrutural da dualidade:
...
As limitações francas, em ordem aproximada de quanto limitam as alegações:
