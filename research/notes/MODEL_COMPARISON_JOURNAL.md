...
- **Entry 27** (pre-registration draft: claiming blind route prediction — provenance-gated
  inventory of historical planning-time predictions + a prospective arm; harness
  `blind_prediction.py` to follow) — this commit

---

## 2026-07-24 — Entry 27: pre-registered draft — claiming blind route prediction (the planning-mode benchmark)

*Prompt (Danilo, reacting to the article's "every accuracy figure measures the consistency of the
energy accounting — not blind route prediction"): "We may want to pre-register E27 already. I have
used that model for blind route prediction before. We may want to think about a protocol for
claiming that." Status: protocol DRAFT — the inventory phase, inclusion rules and freeze points are
declared below; Danilo confirms or amends the prediction sources BEFORE any outcome is unblinded.
No number is computed in this entry.*

**Why this entry exists.** The article's first limitation (§10.4) is that both engines are
conditioned on each ride's measured power, so the published 4–7% medians measure energy-accounting
consistency, not blind prediction — "a true planning-mode benchmark (power guessed before the ride)
remains uncomputed." But blind predictions HAVE been made and recorded in the ecosystem over months
of real planning. Claiming them requires a protocol that cleanly separates genuine pre-ride
predictions from anything that could have been retrofitted.

### Phase 0 — the inventory (no outcomes examined)

Candidate sources of historical blind predictions, each with its provenance mechanism:

- **amora tours** carrying both `ph:energyEstimate` (recorded at planning time; the RDF store and
  its git history timestamp it) and `ph:measuredEnergy` — the cleanest source, with
  machine-checkable provenance.
- **Planning spreadsheets** (censo and others): planned-energy cells whose sheet revision history
  shows they predate the ride.
- **Ad-hoc saved outputs** (quilojaules/sampasimu screenshots, messages, notes) — only where a
  dated artifact exists.

Inventory output, per prediction: the route; the predicted kJ; the model variant and constants
used *at the time* (reconstructed as far as possible — version drift is documented, not repaired);
the provenance evidence that the prediction predates the ride; and whether a measured `∫P·dt`
exists for the realized ride. **FREEZE POINT 1: the inclusion list is fixed, from provenance alone,
before any measured energy is joined to it.**

### Design rules (declared now, before the inventory)

- **Provenance rule.** A prediction counts as *blind* only with document- or machine-level evidence
  it predates the ride (git commit, RDF/store timestamp, sheet revision, dated message).
  Recollection alone goes to a separate, clearly-labeled "self-reported blind" stratum — reported,
  never headlined.
- **Contamination rules.** Predictions revised after riding are excluded; rides whose route
  materially diverged from the predicted route are excluded by a pre-stated geometry criterion
  (route overlap ≥ 80% by distance) applied before unblinding; model-version drift is disclosed
  per prediction.
- **The self-fulfilling-pacing confound, stated up front.** Collective rides are *planned from the
  prediction* (duration at a reference power), so riders may partially conform to it. The claim
  this protocol can support is therefore "the blind prediction is consistent with the energy the
  ride actually realized, under the planning it induced" — not causal forecasting accuracy. Group
  rides additionally confound one rider's `∫P·dt` via drafting (the Entry 20 rule): solo rides
  carry the headline; group rides are a reported stratum.
- **Endpoints.** med |Δ%| and signed bias of the recorded prediction vs measured `∫P·dt`, with
  Entry 22's bootstrap-CI and sign-test machinery; strata: provenance class × solo/group ×
  open/urban. **Registered prediction (a distribution, per the Entry 25 lesson): the pooled blind
  median lands in [7%, 25%] — worse than the power-conditioned 4–7%, since blind adds the
  power-guess error; no bias-sign prediction is registered (no strong prior).**
- **Prospective arm (starts at this registration).** From this date, every planned ride's
  prediction is recorded *before* the ride in amora (`ph:energyEstimate`, timestamped by the
  store), with the constants used noted; evaluation after ≥ 20 realized rides with power, same
  endpoints. This arm is immune to the retrospective arm's provenance limitations and is the
  durable answer to §10.4's gap.
- **FREEZE POINT 2: the analysis harness and its gates are written and committed before outcomes
  are joined** (the same discipline as Entries 19–21).

**Tooling plan.** Phase 0 inventory: a small reader over amora's `tours.ttl`/git history plus a
hand-audited sheet/artifact list (privacy: routes and dates stay out of the public outputs;
aggregates only, as always). Analysis harness: `blind_prediction.py` beside the other harnesses,
gated, import-safe.
