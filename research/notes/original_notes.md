...
- $\epsilon$: typically between 10% and 30%
  - Rough geometric estimate $\epsilon \approx \min(1, \frac{\alpha}{\beta \bar{s}})-\epsilon_0 \pm 0.1$
    with the **coasting deficit** $\epsilon_0 \approx 0.13$ (calibrated in-sample on the 44 power
    rides; on the quasi-independent censo set a flat $\epsilon\approx0.20$ did as well or better —
    see the journal).
...
rides (see `research/journal/MODEL_COMPARISON_JOURNAL.md`):
...
gravity+rolling+aero to within 4% (ratio 0.97, $k_h(\text{sustained}) = 0.96$; journal Entry 7,
2535 sections over 44 rides). So the
...
*measured* flat speed — `src/harness/eps_hypothesis.py`, Journal Entry 8):

- **The grade law holds where $\epsilon$ matters.** Correlation with the measured
  $\epsilon$ climbs from $0.30$ over all rides to $0.77$ on $\bar s\ge 3\%$ and $0.82$ on
  $\bar s\ge 3.5\%$; descent-energy-weighted ($w=\beta H_-$) it is $0.60$.
- **A near-constant $-0.13$ offset** calibrates it. We call it the **coasting deficit**
  $\epsilon_0$: the share of the descent that pure coasting *would* have refunded but the
  rider does not collect, because they keep pedalling into the descent and brake before the
...
- **The clamp-to-$1$ limit is reversed on flat terrain.** Gentle rides are pedalled
  *through* the dips, so the measured $\epsilon\to 0$, not $1$ (NS3 Caracaí: predicted
  ...
  energy, which is why energy-weighting alone lifts the correlation $0.30\to0.60$.
...
*Worked example* (RMC200 Mogi): $\alpha/\beta=0.0204$, $\bar s=3.4\%$ ⇒
$\min(1,\,0.0204/0.0341)=0.60$; minus $0.13$ ⇒ $\mathbf{0.47}$, vs. measured $0.47$.
...
with $\epsilon(s) = \mathrm{clamp}_{01}\!\big(\min(1,\,(\alpha/\beta)/s) - \epsilon_0\big)$ and
the coasting deficit $\epsilon_0 = 0.13$. Two structural facts, both load-bearing:
...
(Entry 19; the coasting deficit was calibrated on **30 m** cells). Deployed mitigations:
...
champion and the canonical simulation are at statistical parity (medians 3.5% vs
5.2% on the longões, overlapping CIs, sign test $p = 0.65$); v2Edge ties the champion
