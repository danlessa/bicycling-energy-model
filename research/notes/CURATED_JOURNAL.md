...
The remaining +8.5% mostly traces to v_f, the flat reference speed. Deriving
it from extracted flat power over-estimates the real flat speed; using the
**measured** flat speed cuts the residual to **+2.7%**.
[[package]](../packages/entry04/ro-crate-preview.html)

### Entry 5 — The elevation data was lying (the champion is born)
Splitting the error by regime shows it lives entirely on climbs — and ~20% of
recorded "ascent" is sub-3-metre altimeter jitter, which the linear β·h₊ term
pays in full (~93% of the residual climb over-charge). A **2 m deadband
filter** on the profile fixes it: the corrected closed form reaches
...
simulation, which is nearly immune to ascent noise.
