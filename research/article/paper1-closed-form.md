...
The claim this licenses is narrower than "replace $\varepsilon_0$". Under [§3.3](#3.3)'s rule the dynamic estimator governs only the descent-bearing half of the corpora; the rest is handled by the flat $\varepsilon_f = 0.20$, which owes nothing to eq. (8). The defensible statement is therefore: **where the dynamic estimator applies at all, its deficit should be grade-inverse rather than constant.** Eq. (8)'s constant is also ungated at the time of writing (lab journal, Entry 45), so it is offered as a result about the deficit's *form*, not as a replacement number for the published tables.
