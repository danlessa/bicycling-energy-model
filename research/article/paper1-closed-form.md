...
# A Closed-Form Model for the Mechanical Energy of Cycling a Route, Tested on 2,025 Power-Meter Rides
...
4. **The coasting deficit as a *family*, not a number.** Its ledger identity makes the choice of form a claim about pedalling *occupancy* rather than effort; four forms are contested out of sample ($\varepsilon_0$ constant, $\varepsilon_1$ per rider, $\varepsilon_2$ grade-inverse, $\varepsilon_3$ encounter fraction). The deficit's *sign* recurs on all seven riders, its *value* does not — 0.12–0.14 on the São Paulo three, 0.08–0.30 with D6's Europeans — and the spread tracks descent pedalling ([§3.2](#3.2)–[§3.5](#3.5), [§4.4](#4.4)).
5. **A grade-inverse deficit that beats the constant.** $\varepsilon_2$: $\delta = k/\bar s$, $k = 0.0051$, one universal constant and no rider parameter, cutting held-out error from 0.067 to 0.055 on 513 rides and winning 344 of them — which reinterprets $\varepsilon_0 = 0.13$ as that form evaluated at the calibration corpus's own terrain. Established only for mean descent grades ≥ 3%, the regime $\varepsilon_d$ is used in ([§3.2.1](#3.2), eq. (8)).
...
7. **External validation on an open deposit.** The frozen law carried to four European riders (Catalonia, Burgundy, the French Alps) who share no rider, country, terrain regime, recording device or model-selection history with the calibration set — and reaching its closest parity there, 3.16% against the simulation's 3.15% on 740 rides. The same corpus shows F4's scalar $c$ failing where the *form* does not, which is the sharpest evidence that $c$ belongs to the elevation source rather than to cycling ([Table 1](#tab1), [Table 3](#tab3)).
...
| $\varepsilon_0\ldots\varepsilon_3$ | — | see [§2.2](#2.2) | coasting-deficit forms | The candidate forms of $\delta$: $\varepsilon_0$ frozen constant, $\varepsilon_1$ rider constant, $\varepsilon_2$ grade-inverse $k/\bar s$ (eq. (8)), $\varepsilon_3$ encounter fraction. Contested out of sample in [§3.2.1](#3.2). |
| $\varepsilon_0$ | — | 0.13 | coasting deficit ($\varepsilon_0$ form) | Gap between ideal and measured recovery; 0.13 at the shared priors and 30 m scale (0.10–0.19 across plausible physics and pairings, [§3.4](#3.4)–[§3.5](#3.5)); recurrence robust, value conditional. |
...
Real riders do not ride the coasting limit: they pedal into descents, so their measured recovery sits below the bound by construction. (Braking, counter-intuitively, cancels out of the measured balance — it dissipates gravity's share of the ledger, never the legs' — so the shortfall is carried by descent pedalling, with braking acting only indirectly through the re-pedalling it forces; [Appendix A](#appendix-a).) Our hypothesis is that this shortfall is a **deficit function** $\delta$ — the **coasting deficit** — subtracted from the bound, so that the working estimator is

$$\varepsilon_d(s_i) \;=\; \varepsilon_{\mathrm{coast}}(s_i) - \delta \tag{4}
$$

$$\varepsilon_d \;=\; \frac{\sum_i h_{-,i}\,\varepsilon_d(s_i)}{\sum_i h_{-,i}} \tag{5}
$$

Eq. (4) is the per-segment statement — the recovery a descent cell of grade $s_i$ should show; eq. (5) drop-weights it over the route's 30 m descent cells (drops $h_{-,i}$) into **one scalar per route**, the $\varepsilon_d$ every table scores. Writing the deficit as $\delta$ rather than as a number is the substantive move. **What functional form $\delta$ takes is an empirical question, not an assumption**, and this paper treats it as one: [§2.2](#2.2) states the candidate forms and the out-of-sample protocol that decides between them, and [§3.2](#3.2) reports the contest. The simplest candidate is a **constant**, $\delta = \varepsilon_0$, calibrated once against power-measured descent balances and then frozen; it is the form every table in [§3](#3.1) is scored under, and it is an excellent constant. But it is only the first member of a numbered family $\varepsilon_0$–$\varepsilon_3$ ([§2.2](#2.2)), not the hypothesis itself, and [§3.2.1](#3.2) finds $\varepsilon_2$ the better member. Every form we test is constant *within* a ride — a ride-level summary — so eq. (5) collapses to the route's drop-weighted $\varepsilon_{\mathrm{coast}}$ minus $\delta$ regardless of which form is chosen; only the per-cell variants of [§3.2.1](#3.2) break that collapse, and they lose.

The deficit is not a free-floating correction. [Appendix A](#appendix-a) gives its exact ledger identity, $\delta = E_{\mathrm{legs},-}/(\beta h_-)$ — **the deficit *is* the descent pedal energy, over the scaled drop** — which as a ratio of two powers reads $\delta = k_{\mathrm{eff}}\bar P_{\mathrm{desc}}/(m g \bar s \bar v)$: what the legs add over what gravity releases. At 80 kg on a 5% descent at 36 km/h gravity supplies ≈ 390 W, so $\varepsilon_0 = 0.13$ is ≈ 50 W of pedalling. That identity factors the deficit into a **magnitude** and a **probability**, $\bar P_{\mathrm{desc}} = \mathrm{occ}(s)\,I(s)$, and the two are not symmetric: magnitude is nearly pinned — riders who pedal a descent do so at essentially their own flat power — while occupancy spans ninefold between riders and falls steeply with grade. Descent pedalling is close to binary: coast, or ride normally. **Choosing a form for $\delta$ is therefore a claim about occupancy, not about effort** — a constant $\delta$ asserts that the share of descent time spent pedalling is route-independent, which the identity's $1/s$ immediately makes doubtful and suggests a competitor: a deficit scaling inversely with descent grade. Both forms are carried to [§2.2](#2.2) and scored in [§3.2](#3.2).

We call this estimator the **dynamic $\varepsilon$**, written $\varepsilon_d$ — it adapts to each route's descent geometry — in contrast to a **flat $\varepsilon$**, written $\varepsilon_f$: a single constant for every route, the alternative it is scored against throughout.

The estimator is deliberately *unclamped*: with a constant deficit it lives in $[-\varepsilon_0,\, 1-\varepsilon_0]$ by construction ($0 \leq \varepsilon_{\mathrm{coast}} \leq 1$), and a negative prediction — possible only on mean descent grades beyond $s_*/\varepsilon_0 \approx 15\%$, steeper than anything in the corpora — is not unphysical either, since measured recovery *can* go negative (a rider who pedals a descent harder than the flat bill). Under the grade-inverse deficit of eq. (8) that bound is replaced by $[-k/\bar s,\, 1-k/\bar s]$, which is why eq. (8) carries an explicit domain ([§3.2.1](#3.2)); within it the two deficit forms differ by less than 0.05 and $\varepsilon_d$ stays comfortably inside $(0,1)$. On the 1,300-odd rides where the estimator is defined it never left $(0, 1)$; [Appendix A](#appendix-a) states the exact bounds, and a grade-resolved refinement that *fades* the deficit on steep grades was tested and resolved in [§4.4](#4.4): the fade is the confirmed mechanism at segment grain, and the constant is its correct ride-level summary. What the deficit *is* — geometry or habit — and whether it transfers across riders are empirical questions, answered in [§3.2](#3.2)–[§3.3](#3.3).
...
**Forms of the deficit, and how they are contested.** Because [§1.3.2](#1.3)'s identity makes $\delta$ a ratio of powers, "$\varepsilon_0$ is a constant" is one candidate among several, and we score it against the alternatives rather than assuming it. Four are carried forward, numbered $\varepsilon_0$–$\varepsilon_3$ as the forms are numbered F1–F4:

1. $\varepsilon_0$, the **frozen constant** — $\delta = \varepsilon_0$, one universal number, the incumbent;
2. $\varepsilon_1$, the **rider constant** — one number per rider, no route dependence;
3. $\varepsilon_2$, the **grade-inverse** — $\delta = k/\bar s$, carrying the identity's $1/s$ to the ride's drop-weighted mean descent grade $\bar s$;
4. $\varepsilon_3$, the **encounter fraction** — $\delta = a + b\varphi$, with $\varphi$ the share of a ride's drop falling below the rider's own half-pedalling grade.

They differ sharply in cost, which is why the contest is decided on held-out error rather than fit: $\varepsilon_2$ needs one route statistic and one universal constant, $\varepsilon_1$ one fitted number per rider, and $\varepsilon_3$ both a fitted per-rider threshold and the route's full grade distribution.
...
6. **large-sample in-sample validation** — deliberately *not* independent (the calibration rider's complete history): it validates the machinery at scale — mass inversion against a known weight, the filters, the parser — not the law.
...
**Table 2.** Calibration-corpus scoreboard (44 rides): median $\lvert\Delta\%\rvert$ [95% CI] and median signed $\Delta\%$. Upper block: the calibration run — condition-informed per-ride parameters (logged mass, identical in both runs; judged $C_{rr}$, $C_dA$, air density and head/tailwind; a hand-chosen per-ride $\varepsilon$; [§2.3](#2.3) gives the ranges). Lower block (italic): the same forms re-run *blind* under the frozen protocol every other corpus uses — one shared literature-typical constants set, dynamic $\varepsilon_d$ — in the upper block's row order, so each form's informed→blind shift reads straight down. Lineage ([Terminology](#terminology)): $O = F^{\varepsilon}(D_1, P_{a,r})\,|\,\sigma$ upper, $F^{\varepsilon_d}(D_1, P_{a,g})\,|\,\sigma$ lower, over $F_1$–$F_4$ and $F_{\mathrm{base}}$, with $\sigma$ = the 44 power-metered longões; $|O| = 44$ in both blocks, the same rides throughout.
...
Scored out of sample on 513 held-out rides across all six corpora, D6's four riders counted individually — **every one of them with a mean descent grade of 3% or more**, the regime $\varepsilon_d$ is used in, so what follows is a statement about that regime and not about gentler terrain (lab journal, Entry 45) — the frozen $\varepsilon_0 = 0.13$ is confirmed as an excellent *constant* — it is statistically indistinguishable from the best single constant refitted on these data ($p = 0.86$) — but it is beaten by $\varepsilon_2$, the grade-inverse form, which lets the deficit fall with descent grade:

$$\delta(\bar s) \;=\; \frac{k}{\bar s}, \qquad k = 0.0051 \tag{8}
$$

with $\bar s$ the ride's drop-weighted mean descent grade as a fraction. **Eq. (8) is defined only on real descents, $\bar s \geq 3\%$** — the regime the dynamic estimator is *recommended* for ([§3.3](#3.3)'s rule), and the only range over which eq. (8) was either fitted **or scored** (the contest's rides span $\bar s$ = 3.00–9.69%, median 3.70%; none lies below 3%). **Below 3% the comparison is untested**: the ride-level contest has no observations there, and the separate flat-terrain probe that does reach 0.78% measures the *unclamped* ledger identity rather than the clamped quantity $\varepsilon_0$ is defined on, so it cannot adjudicate $\varepsilon_2$ against $\varepsilon_0$ in this paper's terms (lab journal, Entry 45 amendment).

#### 3.2.2 Scope, and what eq. (8) licenses

**The restriction is a stated scope condition, not an enforced one:** the evaluation behind [Table 3](#tab3) applies $\varepsilon_d$ to every ride in a corpus, including the **69%** whose own mean descent grade falls below 3% (940 of Table 3's 1,366 rides), because no per-ride switch is implemented. Eq. (8) is therefore conditional on the rule being applied, and adopting it would require implementing the switch and re-running the grid — which would move Table 3's $\varepsilon_d$ column. We report the form and its scope; we do not restate the corpus medians under it. The restriction is not cosmetic: $k/\bar s$ diverges as the terrain flattens, exceeding 1 below $\bar s = k \approx 0.5\%$, where a "deficit" larger than the whole refund is meaningless. Outside the regime the flat $\varepsilon_f = 0.20$ applies, exactly as before. The divergence is the identity's, not an artefact of fitting: gravity releases power in proportion to $\bar s$, so as descents flatten any residual pedalling becomes an ever-larger *share* of an ever-smaller release. At the energy level the term stays finite — the refund $\beta(\varepsilon_{\mathrm{coast}} - k/\bar s)h_-$ equals $\beta(\varepsilon_{\mathrm{coast}}\bar s - k)x_-$, which tends to $-\beta k x_-$ — but the *fraction* does not, and $\varepsilon$ is a fraction.
...
The claim this licenses is narrower than "replace $\varepsilon_0$". Under [§3.3](#3.3)'s rule the dynamic estimator governs only the descent-bearing third of the corpora; the rest is handled by the flat $\varepsilon_f = 0.20$, which owes nothing to eq. (8). The defensible statement is therefore: **where the dynamic estimator applies at all, its deficit should be grade-inverse rather than constant.** Eq. (8)'s constant is gated in the project's verification battery (lab journal, Entry 45), but it is still offered as a result about the deficit's *form*, not as a replacement number for the published tables, because adopting it requires the unimplemented switch above.

Two forms with more parameters were also tested and do **not** displace eq. (8) in practice. $\varepsilon_1$, the per-rider constant, reaches 0.047 but needs one fitted number per rider; $\varepsilon_3$, the encounter fraction, reaches 0.043 but needs a fitted per-rider threshold *and* the route's full grade distribution. Both are stronger on accuracy and far weaker on deployability, and neither is available to a planner meeting a new rider. Two further forms — evaluating the physical curve at the mean grade, and integrating it per cell — are *decisively worse* here despite winning on a closely related quantity, because they are built from the unclamped identity while $\varepsilon_{\mathrm{coast}}$ is clamped: a reminder that a deficit model must be fitted on the quantity it will be subtracted from.
...
<tbody>
<tr><td>F3 · ε<sub>d</sub></td><td>7.7 [6.0,9.3]</td><td>−5.1 [−7.6,−2.2]</td><td>5.8 [5.3,6.4]</td><td>+4.3 [+3.1,+4.9]</td><td>5.5 [4.4,6.4]</td><td>−4.7 [−5.7,−3.7]</td><td><strong>6.2</strong> [5.6,6.9]</td><td>−0.3 [−1.6,+0.6]</td><td><strong>3.2</strong> [2.9,3.5]</td><td>+2.0 [+1.6,+2.3]</td><td><strong>5.9</strong> [5.5,6.2]</td><td>+0.4 [−0.1,+1.1]</td><td>4.9 [4.7,5.1]</td><td>+1.3 [+0.9,+1.6]</td></tr>
<tr><td>F4 · ε<sub>d</sub></td><td><strong>6.4</strong> [4.8,8.6]</td><td>−3.4 [−4.9,−0.3]</td><td><strong>4.9</strong> [4.4,5.8]</td><td>+0.6 [−0.1,+1.3]</td><td>9.0 [7.9,9.7]</td><td>−8.4 [−9.5,−7.5]</td><td>7.1 [6.4,8.1]</td><td>−1.9 [−3.0,−1.4]</td><td>4.0 [3.8,4.4]</td><td>−2.2 [−2.6,−1.9]</td><td>6.6 [6.3,7.1]</td><td>−2.4 [−3.0,−1.9]</td><td>5.3 [5.0,5.6]</td><td>−2.3 [−2.7,−2.0]</td></tr>
...
**The coasting deficit recurs on every rider.** P. Paz's measured gap between $\varepsilon_{\mathrm{coast}}$ and $\varepsilon_{\mathrm{bal}}$ on real descents is 0.12 [0.10, 0.14]; JAAM's is 0.13 [0.10, 0.19]; the author's full history gives 0.14 [0.12, 0.16]; the calibration value was 0.13. D6's four Europeans widen that considerably — 0.117, 0.298, 0.080 and 0.175 — so across all seven riders the gap's *sign* is universal while its *value* spans 0.08–0.30, and the wider spread tracks how much each rider pedals his descents (lab journal, Entry 43). (Positivity of the gap is structural only at the true physics — computed under assumed constants it is an empirical finding, and the sweep's implausible low-$\rho C_dA$ corners can invert it ([§3.4](#3.4)) — so we count the recurrence as *consistent across riders*, not as three independent confirmations.)
