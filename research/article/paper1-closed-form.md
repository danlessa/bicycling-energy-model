...
Below the threshold the coasting limit clamps, so the $\varepsilon_d$ actually applied has median 0.544<!--@c-e46.eps_applied.sub3--> against the flat constant's 0.20. That is a large refund, and under the frozen priors it very nearly cancels the over-prediction of $C_dA = 0.40$ — a value well above the 0.26–0.32 these riders invert to. The result is a sub-threshold bias of +0.28 that looks like accuracy. Score the same rides under per-ride inverted physics and the cancellation disappears: the bias falls to −4.64, while the flat constant, badly biased under the frozen priors (+5.90), becomes the near-unbiased choice (+0.11).
