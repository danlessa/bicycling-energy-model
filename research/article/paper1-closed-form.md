...
6. **Evidence that the descent constant is a property of cycling, not of a rider.** Fitted on one person's rides and carried unchanged to six others who took no part in fitting it, it costs 0.05 pp of median accuracy, inside the margin registered in advance as the threshold for mattering — so the published number serves a user who contributed nothing to it. The corpus that makes this testable is an open European deposit (Catalonia, Burgundy, the French Alps) sharing no rider, country, terrain regime or recording device with the São Paulo data.
...
> **Transfer.** The descent-recovery constant $\varepsilon$ is a property of a riding **context** — a landscape and the terrain regime it imposes — rather than of a rider. Within one context it should carry **unchanged** from the people it was fitted on to people who took no part in fitting it. Across contexts it should carry with a **change to $\varepsilon$ alone**: the law, the choice of form, and every other constant stay put.

The two halves fail differently and are tested separately. If the first fails, $\varepsilon$ is a per-rider calibration, the law needs a measurement session per person before anyone can use it, and the deployment case in [§4.1](#4.1) largely collapses. If the second fails — if moving to a new landscape required re-deriving the law rather than re-fitting one number — then nothing here transfers beyond the roads it was measured on, and the paper would be a case study rather than a method.

We test both. Fitting $\varepsilon$ on a single rider and scoring it on six others who share their context costs 0.05 pp<!--@c-e54.transfer-->, inside the margin registered in advance as the threshold for mattering. Carrying it between the two landscapes in these corpora — the São Paulo corpora against the European deposit — costs about 0.85 pp if the difference is ignored, and the two contexts want $\varepsilon \approx 0.24$ and $\approx 0.37$ respectively. So the constant does travel between people, and between places it travels as a *re-fitted number* rather than as a re-derived model. That is the weaker claim, and it is the one the evidence supports.
...
**Is that terrain or is it people?** The corpora confound the two — the São Paulo riders are not the European ones — so the empirical split alone cannot say. A synthetic sweep settles it: running the reference simulation over both regions' *real route geometries* with a single fixed synthetic rider, and fitting each form's parameters jointly to the result, reproduces a gap of **+0.125** against the +0.133 measured on riders. With behaviour held identical across regions, **94% of the difference survives**, so it is a property of the landscapes rather than of the people who ride them (lab journal, Entry 61). That is what licenses pooling by region rather than by rider, and it is why [§4.4.3](#4.4.3) treats a landscape-class table as a well-founded programme rather than a hope.

Two caveats travel with this. Two landscapes are two data points, so the *values* are measurements rather than a table a reader can look their own terrain up in; [§4.4.3](#4.4.3) sets out what turning them into one would require. And the totals-only form carries part of the landscape dependence in its scalar $c$ rather than in $\varepsilon$ — under the synthetic sweep $c$ comes out at 3.02 on D3–D5 against 1.84 on D6 while the deadband is 4 m in both — so a user working from route totals adjusts a different number than one working from a profile.
...
**Where the refund physically travels.** One further piece of the mechanism, derived and validated after the results above froze (lab journal, Entries 63–69): part of a descent's release leaves the descent *in transit as kinetic energy* — the entry/exit boundary term that [Appendix A.2](#A.2) defers to ride-level aggregation — and is collected on the following terrain over a computable **recovery length**, $L_{\mathrm{rec}} = m v_f^2/(C_{rr} m g + 3\cdot\tfrac12\rho C_dA\,v_f^2) \approx 90$ m at this study's constants: a climb collects it as free lift, a run-out collects it against the flat bill over two to three such lengths, a stop destroys it. The recovery length is thus the spatial footprint over which $\varepsilon$'s credit is realised, and truncated collection — interruptions denser than the footprint — is one identified component of the deficit alongside downhill pedalling. This is also the mechanism that connects $\varepsilon$ to the smoothing scale of [§4.4.4](#4.4.4): the same kinetic buffer, seen from the descent side, is the refund in transit; seen from the climb side, it is the momentum payment the deadband anticipates.

#### 4.4.3 Tabulating ε by landscape class

This paper measures $\varepsilon$ in two landscapes and finds two values — about 0.24 across the São Paulo corpora and 0.37 across the European deposit, a gap that costs 0.85 pp of accuracy if ignored ([§1.3](#1.3)). Two points is not a theory. It is, however, enough to say what the next step is: **$\varepsilon$ should be tabulated against landscape classes rather than fitted afresh per corpus**, and that is a programme rather than an afterthought.

The mechanism sketched in [§4.4.2](#4.4.2) already predicts the direction. The coasting ceiling $\varepsilon_{\mathrm{coast}}(s) = \min(1, (\alpha/\beta)/s)$ falls as the descent steepens, so a landscape's $\varepsilon$ should depend on the *distribution* of its descent grades and lengths, not on a single mean. São Paulo's descents are short, shallow and interrupted by traffic; the Alpine and Catalan descents in D6 are long and sustained, which is exactly where a rider recovers most of what gravity offers. A landscape descriptor that predicts $\varepsilon$ therefore needs to carry descent-length distribution, grade distribution, and something for interruption density — the last being the one this study has no instrument for.

The programme has two halves, and neither works alone. **Simulation** can sweep $\varepsilon$ over synthetic and DEM-derived terrain far beyond the two landscapes measured here, producing a predicted table at low cost; the forward model in [§2.1](#2.1) is already the instrument for it, since it computes descent recovery from first principles rather than from a fitted constant. But simulation inherits its own assumptions about how riders brake and coast, and those assumptions are precisely what $\varepsilon$ was introduced to absorb. **Empirical validation** on corpora from further landscapes is what turns a predicted table into a usable one, and the requirement is more corpora rather than more rides: seven riders in two places is two data points for this question, whatever it is for the others.

What would make the table worth publishing is a **descriptor computable from a map**, since a planner choosing $\varepsilon$ for a route has terrain and no power meter. A classification that needed ridden data to assign a route to its class would answer the wrong question. That constraint — map-derivable, not ride-derived — is what makes this a genuine research problem rather than a curve-fitting exercise, and it is why it belongs in a later paper rather than a longer version of this one.

#### 4.4.4 The deadband decomposed: momentum physics, a measured floor, and a fitted residue

The suspension reading (lab journal, Entries 37–40) reinterprets the smoothing scale itself. The kinetic energy of cruising is worth a height $h_{KE} = v^2/2g$ — 2.5 to 6.3 m at 25–40 km/h — so the rider–terrain system behaves as a travel-limited suspension: the KE ↔ PE exchange over a roller is the spring, drag along the traverse is the damper (excess speed decays with the exact, $C_{rr}$-free length $\lambda = m/\rho C_dA \approx 170\text{–}230$ m; wind rescales it by $v/(v+w)$), and bumps beyond the travel bottom out onto the legs. The fitted $\tau = 6$ m sits at buffer scale, far above the ~2 m jitter floor of [§2.5](#2.5) — and the identification can be made exact: traced through one oscillation, the backlash filter annihilates swings under $2\tau$ peak-to-peak and counts every survivor short by exactly $2\tau$, a fixed-cap, unconditional realisation of the same buffer.

A follow-up programme (Entries 63–69) took the identification constructively: replace the fitted cap with a per-valley toll $\min(D, H, h_{KE})$ computed from the dynamics' own fixed points — the descent's terminal speed, the flat equilibrium, the next climb's quasi-steady speed. The toll alone reproduces about half of the filter's cross-validated benefit over the unfiltered form, and the two together about four-fifths; of what the toll explains, roughly seven-eighths the filter also explains — they are substantially the same term. Every measurement-error attribution for the filter's remaining share was then tested and acquitted: hierarchical (rainflow-style) valley pairing over-tolls; Gaussian pre-smoothing is inert; and a closure-pair probe — same-place, different-time revisits within each ride, no DEM anywhere — finds real barometric drift (median 2.8 m, 3.4 m/h) whose per-ride amplitude does not predict where the larger filter helps.

What the fitted refinement buys beyond a *fixed* floor is, on inspection, exactly the part that does not generalise: the per-rider optimal $\tau$ is non-stationary (it moved in five of six within-rider early/late splits), constants fitted on a rider's early half serve their later rides several-fold worse than the mechanistic form's do, and leave-one-rider-out transfer erases the in-pool edge. Combined with the flat basin of [§3.2](#3.2) this closes into one reading, the same one that governs $\varepsilon$: **where the loss is locally flat, fitting harvests noise, and fitted noise is precisely what fails to transfer or persist.** The deadband's threshold is therefore best read as a calibrated correction standing beside $\varepsilon$ — a knob with an identified physical core and a quantified fitted residue — not as a physical filter constant.

The constructive endpoint is a witness, not a replacement. A variant with *no chosen geometry constants* — each corpus's floor pinned by its own measured drift amplitude, the toll on top, the brake cap frozen at the arm the data rail to, $\varepsilon$ the single fitted scalar — matches the published form's cross-validated accuracy and transfers across riders better than anything else tested (per-ride sign test $p = 10^{-4}$; Entry 69). The refinement registered in an earlier version of this section — a speed-dependent $\tau \approx \eta\,v_f^2/2g$ — was tested at rider grain and its *ordering* fails (the per-rider optimum tracks residual bias, not buffer height; Entry 64); what survives is the magnitude statement and the measured-floor prescription. The published constants stand; the decomposition is what licenses reading them as calibrated corrections with identified causal sources. The derivation-to-validation chain is collected in the project's `epsilon-origin` note.

#### 4.4.5 Per-rider physics without circularity
