...
Scored out of sample on 513 held-out rides (lab journal, Entry 45 — the contest spans nine rider-corpora, four of them an external deposit this paper does not otherwise use, so it is reported here as a journal result about the deficit's *form* rather than as a corpus result of this study), the frozen $\varepsilon_0 = 0.13$ is confirmed as an excellent *constant* — it is statistically indistinguishable from the best single constant refitted on these data ($p = 0.86$) — but it is beaten by a form that lets the deficit fall with descent grade:
