...
Eq. (4) is the per-segment statement — the recovery a descent cell of grade $s_i$ should show; eq. (5) drop-weights it over the route's 30 m descent cells (drops $h_{-,i}$) into **one scalar per route**, the $\varepsilon_d$ every table scores. Since $\varepsilon_0$ is constant, eq. (5) equals the route's drop-weighted $\varepsilon_{\mathrm{coast}}$ minus $\varepsilon_0$. (A lumped shortcut — eq. (3) at the mean descent grade — was tested as a hand proxy and rejected: it under-refunds systematically; the hand recipe of [§4.1](#4.1) therefore uses the flat constant. Lab journal, Entry 42.) The constant $\varepsilon_0$ is calibrated once against power-measured descent balances ([§2.2](#2.2), [§3.2](#3.2)) and then frozen.
...
This yields 74.5 kg [IQR (interquartile range) 69.1–78.4] for D3, 101.9 kg [95.9–109.0] for D4, and 74.7 kg [67.7–81.0] for D5 — validated against the author's known ≈ 73 kg ([§3.4](#3.4), [§3.5](#3.5)). Note $\hat m_p\,g$ is invariant under a change of gravity constant by construction.
...
> 4. **Choose $\varepsilon$**: use the flat $\varepsilon_f = 0.20$. (It under-refunds open descents, so the estimate errs safe — it overestimates demand. The dynamic $\varepsilon_d$ needs profile software, eq. (5); the mean-grade pencil shortcut was tested and rejected — journal Entry 42.)
