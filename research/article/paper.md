...
**Methods.** We test four models built on one three-term closed form, $E \approx \alpha\,x + \beta\,(h_+ - \varepsilon\,h_-)$ — a flat cost rate $\alpha$ per metre of distance $x$, a climbing rate $\beta$ per metre of ascent $h_+$, and a refund of the fraction $\varepsilon$ of the descended height $h_-$ — against the mechanical energy $\int P\,dt$ measured by power meters on 1,285 unique rides (1,387 ride-evaluations across five overlapping corpora) from three riders in São Paulo — the urban corpus re-evaluates the author's own recordings under a generic-rider assumption. The reference is a forward-dynamics simulation of the standard cycling power balance [Martin et al. 1998], run on the same physical constants, so every gap between the two is the model's fault, not the parameters'. Each ride's measured power feeds both models, so accuracy here means consistency of the energy accounting, not blind prediction. The calibration corpus is evaluated with condition-informed per-ride parameters (mass, rolling and drag coefficients, air density, head/tailwind, and $\varepsilon$, judged per ride) and re-run blind as a check; every other corpus is blind under one shared literature-typical constants set. Every behavioural constant is calibrated once and frozen; the only per-rider input is total mass — logged on the calibration corpus, inverted from each independent rider's own climbing data, generic on the urban one.

**Results.** The four models have a median error (against calibration measurements, with condition-informed per-ride parameters) of 3.5% [95% CI 2.0, 5.6] (form 3, elevation smoothed), 5.9% [3.6, 8.3] (form 4, elevation corrected), 8.6% [7.2, 11.0] (form 2, split) and 19.1% [17.3, 21.5] (form 1, original), with the reference simulation at 5.2% [3.8, 7.3] — the two corrections carry almost all of the gain (climb aerodynamics charged at the flat reference speed; sub-metre elevation noise counted as lifting work), and the best form is statistically indistinguishable from the simulation (paired sign test $p = 0.65$; $n = 44$ limits statistical power; equivalence not formally tested). Re-run *blind* under one shared literature-typical constants set, the corrected forms cluster at 7.6–8.2% and the simulation at 8.4% ($p = 1.00$): parity is protocol-independent, and the 4–5-point gap prices condition knowledge. For the descent term we derive a coasting-limit expression, $\varepsilon(s) = \min(1, (\alpha/\beta)/s)$ with $s$ the descent grade, and calibrate one constant against power-measured descent balances: the **coasting deficit** $\varepsilon_0 = 0.13$, the share of the coasting refund the rider never collects. Frozen and carried to two independent riders' full histories, the law holds at 3.5–6.2% median error on the rider-transfer and scale corpora with the terrain-appropriate $\varepsilon$ rule (pooled over the three: 5.9% [5.5, 6.2] vs the simulation's 6.2% [5.9, 6.6]; 6.4–7.7% on the urban regime test), and the coasting deficit recurs on every rider — measured gaps 0.12–0.19 under the assumed and per-rider fitted physics, positive across the plausible parameter region of a 108-point sensitivity sweep. The blind protocol is the one every transfer corpus uses.

**Conclusions.** The proposed model is $E = \alpha_r\,x + \alpha_a\,x_{\mathrm{flat}} + \beta\,(\tilde h_+ - \varepsilon\,\tilde h_-)$ — form 3: air resistance $\alpha_a$ charged only over the non-climbing distance $x_{\mathrm{flat}}$, rolling $\alpha_r$ still over all of $x$, smoothed elevation totals $\tilde h_\pm$. It accounts for measured route energy as well as a full simulation does (3.5% [2.0, 5.6] vs 5.2% [3.8, 7.3] on the calibration corpus with its condition-informed parameters; 8.2% vs 8.4% fully blind — parity either way; 3.5–6.2% on the frozen transfer corpora); its approximation form 4, which instead corrects the raw totals by subtracting $c \approx 3$ m of phantom climb per kilometre, performs nearly as well (5.9% [3.6, 8.3] informed, 7.6% [5.6, 11.6] blind) while needing only distance, ascent, descent and a rough climbing-distance share — computable by hand, with no specialized software. Descent recovery has a geometry (the coasting limit) and a habit (the deficit); the deficit constant transfers across riders, while the dynamic term's extra accuracy over a single flat constant is rider- and parameter-dependent — a flat $\varepsilon \approx 0.20$ suffices in urban stop-go riding. The law is cheap enough to serve as a per-edge routing cost at the elevation-sampling scale it was calibrated on.
...
with $\varepsilon_0$ calibrated once against power-measured descent balances ([§2.2](#2.2), [§3.2](#3.2)) and then frozen. We call this estimator the **dynamic $\varepsilon$**, written $\varepsilon_d$ — it adapts to each route's descent geometry — in contrast to a **flat $\varepsilon$**, written $\varepsilon_f$: a single constant for every route, the alternative it is scored against throughout. The clamp's floor is behavioural, not physical: measured recovery *can* go negative (a rider who pedals a descent harder than the flat bill spends extra leg energy there), but coasting always keeps $\varepsilon \geq 0$ achievable, so a behaviour-blind estimator predicts the coastable floor; the floor binds only on mean descent grades beyond $s_*/\varepsilon_0 \approx 15\%$, and [Appendix A](#appendix-a) states the exact bounds. What the deficit *is* — geometry or habit — and whether it transfers across riders are empirical questions, answered in [§3.2](#3.2)–[§3.3](#3.3).
...
**Table 2.** Calibration-corpus scoreboard (44 rides): median $\lvert\Delta\%\rvert$ [95% CI] and median signed $\Delta\%$. Upper block: the calibration run — condition-informed per-ride parameters (mass, $C_{rr}$, $C_dA$, air density and head/tailwind judged per ride, plus a hand-chosen per-ride $\varepsilon$; [§2.3](#2.3) gives the ranges). Lower block (italic): the same forms re-run *blind* under the frozen protocol every other corpus uses — one shared literature-typical constants set, dynamic $\varepsilon_d$.
...
The *blind* re-run (Table 2's lower block) is the coherence check: under the frozen protocol every other corpus uses — one shared literature-typical constants set, zero wind, dynamic $\varepsilon_d$ — the corrected forms cluster at 7.6–8.2% and the simulation at 8.4%, still statistically indistinguishable (form 3 vs simulation: 22 of 44, $p = 1.00$; form 4: 17 of 44, $p = 0.17$). Three readings follow. First, parity is protocol-independent — both engines degrade in lockstep, exactly as the Entry-30 sweep predicted. Second, the 4–5 point gap between the blocks, moving both engines together, is the measured value of condition-informed per-ride judgment over one constants-fit-all set (with the caveat that the hand-chosen per-ride $\varepsilon$ shades into per-ride fitting); the sensitivity sweeps of [§3.4](#3.4) say parameters move absolute error at exactly this scale, and the gap motivates the per-ride-inference and measured-constants routes of [§4.4](#4.4). Third, blind, D1 is the *hardest* corpus — long wind-exposed brevets are where zero-wind generic constants bite hardest — and the transfer corpora, equally heterogeneous but judgment-less, carry that same blind penalty in all their numbers.

Two checks support the attribution. First, on *sustained* climbs (2,535 sections ≥ 3% over ≥ 100 m, 54% of all ascent) the measured energy matches the expected gravity + rolling + aero to within 4% (41,790 vs 43,233 kJ; ratio 0.97 informed; 0.95 frozen) — the gravity term needs no discount; what inflates raw $h_+$ is the noise and rollers the deadband removes. Second, carried frozen to the urban corpus (D2) with a fully generic assumed rider, the law with $\varepsilon_d$ lands at 7.7% median with form 3 (6.4% with form 4) against the simulation's 6.6% — parity again, with nothing refit ([Table 3](#tab3), D2 column, carries the CIs). The flat $\varepsilon_f = 0.20$ does better still there (4.7% / 3.9%, parity with the simulation at $p = 0.70$) — but that constant was itself selected in-sample on these urban rides, so those two medians read as a fitted benchmark, not as transfer; the frozen-variant numbers are the honest headline.
...
**Table 3.** Frozen-constant results on every non-calibration corpus: the urban regime test (D2), the two independent riders (D3–D4, transfer), the author's full history (D5, in-sample machinery validation at scale), and the D3–D5 pool. All four form × ε combinations are frozen; best law per corpus in bold. The flat constant is ε_f = 0.20. Starred cells are in-sample (ε_f was selected on D2 — also why D2 is excluded from the pool: 58 of its 62 rides are already in D5); † marks the in-sample corpus. Brackets are 95% CIs throughout; the pooled row's are stratified bootstrap (rides resampled within each corpus, then pooled). Signed medians print with ties-away rounding; the D5 simulation's exact signed median is +0.05.
...
<tr><th>med |Δ%|</th><th>med Δ%</th><th>med |Δ%|</th><th>med Δ%</th><th>med |Δ%|</th><th>med Δ%</th><th>med |Δ%|</th><th>med Δ%</th><th>med |Δ%|</th><th>med Δ%</th></tr>
</thead>
<tbody>
...
The grid matches the regime rule from [§3.2](#3.2) — the dynamic estimator wins on the open-road rider (D3), the flat constant on the gentle-terrain rider (D4) — and adds a sharper observation: the form × $\varepsilon$ interaction is itself regime-dependent. Form 4 with $\varepsilon_d$ is the *best* cell on P. Paz (4.9, the journal's published headline for that corpus) and the *worst* on JAAM (9.0), where the scalar elevation correction and the dynamic estimator compound on gentle terrain. With the regime-appropriate $\varepsilon$, the law stays at or better than simulation parity on both riders. Adding the other corpora under their stated protocols (D1 3.5% [2.0, 5.6] informed, 8.2% [4.5, 10.8] blind; D2 7.7% [6.0, 9.3] with frozen $\varepsilon_d$ — its regime-appropriate flat constant was chosen *on* that corpus, so its dynamic number is the honest one; D5 6.2% [5.6, 6.9], [§3.4](#3.4)), the law holds at **3.5–6.2% median error on the frozen rider-transfer and scale corpora**, 6.4–7.7% on the frozen urban test, and 3.5% (informed) to 8.2% (blind) on the calibration corpus itself, when the [§3.2](#3.2) regime rule picks the $\varepsilon$ variant. Pooled over D3–D5 ($n = 1{,}281$; stratified bootstrap, rides resampled within each corpus), form 3 with $\varepsilon_d$ reads **5.9% [5.5, 6.2]** against the simulation's 6.2% [5.9, 6.6] — one number for the law's frozen accuracy at scale.

**The coasting deficit recurs on every rider.** P. Paz's measured gap between $\varepsilon_{\mathrm{coast}}$ and $\varepsilon_{\mathrm{bal}}$ on real descents is 0.12 [0.10, 0.14]; JAAM's is 0.13 [0.10, 0.19]; the author's full history gives 0.14 [0.12, 0.16]; the calibration value was 0.13. (The sign is structural only for the *unclamped* bound; for the clamped estimator, positivity is an empirical finding — the sweep's implausible low-$\rho C_dA$ corners can invert it ([§3.4](#3.4)) — and we count the recurrence as *consistent across riders*, not as three independent confirmations.)
...
**In-sample validation at scale.** On the author's 621 clean rides the frozen grid replays the calibration story at fourteen times the sample ([Table 3](#tab3), D5 column): form 3 with $\varepsilon_d$ and the simulation are statistically indistinguishable (6.2 vs 6.1, both with near-zero bias), the flat constant loses on the author's open terrain — the [§3.2](#3.2) regime rule confirmed at scale — and the coasting deficit recurs (measured gap 0.14 [0.12, 0.16] on 210 real descents).

**The [§1.4](#1.4) hypotheses, resolved.** H1 (attribution and parity): confirmed — two mechanisms carry the correction and the corrected forms are statistically indistinguishable from the simulation ([§3.1](#3.1)). H2 (a single constant on genuine descents): confirmed at the shared physics and 30 m scale — the deficit recurs at 0.12–0.14 across riders — but it fails on gentle terrain ([§3.2](#3.2)), and its *value* is conditional on $\rho C_dA$ and sampling scale (the sweep above, [§4.4](#4.4)). H3 (transfer): the law and the deficit transfer; the dynamic estimator's extra accuracy over a flat constant is rider- and parameter-dependent ([§3.3](#3.3)).
...
> 4. **Choose $\varepsilon$**: descents mostly steeper than 3% mean grade → the dynamic $\varepsilon_d = \min(1, (\alpha/\beta)/\bar s) - 0.13$, floored at 0 if negative (only relevant beyond ≈ 11% mean descent grade); urban stop-go or gentle terrain → the flat $\varepsilon_f = 0.20$. This mean-grade form is the lumped approximation of the drop-weighted estimator ([§3.2](#3.2)).
> 5. **Sum**: $E = \alpha_r x + \alpha_a x_{\mathrm{flat}} + \beta h_+ - \varepsilon\,\beta h_-$. The climbing-distance share is the recipe's fourth route input: read it from the profile if you have one, or use $x_{\mathrm{flat}} \approx 0.8\,x$ as a rolling-terrain default (the calibration corpus's median ride climbs for 21% of its distance).
>
