...
> **DRAFT / working paper — Pedal Hidrográfico research notes** (v0.13, July 2026). Numbers and equations are drawn from the project's own benchmarks; novelty claims are corpus-bounded (see §11.3). Accuracy figures are self-reported against three riders' power meters — the author plus two independent riders, not collective members (see §10.4); the ε calibration is in-sample on rider 1, and on the two out-of-sample riders the energy law and −0.13 offset transfer while the geometric ε skill proves rider-dependent (§8.6); the reported ε correlations are part–whole (§8.3); the time model is half-confirmed (ascent transfers, descent bridge does not — §8.8); the assumed rider CdA/C_rr are independently corroborated as plausible (§10.4, journal Entry 15). Not peer-reviewed; circulated for internal review and comment.

**Danilo Lessa Bernardineli** — *Pedal Hidrográfico* (collective), São Paulo, Brazil — danilo.lessa@gmail.com

## Abstract

Planning community bicycle rides needs the *energy* of a route in kJ, but the standard tool is a forward-dynamics simulation [Martin et al. 1998] that is expensive and opaque for planning. We study a cheap closed-form alternative, `E ≈ α·x + β·(h₊ − ε·h₋)`, in which a single lumped factor `ε ∈ [0,1]` captures how much descent potential energy is recovered rather than wasted to excess drag and braking. We give `ε` a closed form in the coasting limit, `ε(s) = min(1, α/(β·s))`, drop-weighted over the profile with a calibrated −0.13 offset, and assess it against a power-meter descent-energy-balance `ε`: on real descents (mean slope ≥ 3%) the calibrated estimator cuts the RMS error by 37% relative to the best flat constant (in-sample; caveats in §8.3), and the offset transfers — calibrated on 44 open rides and applied frozen to 62 urban stop-go rides, it ties the constant selected in-sample there. We test the frozen estimator on **two further independent riders** (different people, different power meters, neither a collective member): the calibrated −0.13 offset recurs on both (measured gaps 0.12, 0.13), but the geometric ε *skill* proves **rider-dependent** — it beats a coasting rider's own best flat constant by ~35% (RMS 0.091 vs 0.139 on real descents), yet is statistically inconclusive for a fast descent-pedalling rider whose measured recovery is low. Run on the *same* physical constants as the simulation, the closed form reproduces measured `∫P·dt` to a 3.6% median over 44 power rides (best variant), to ~4–7% over 62 urban São Paulo rides, and to ~4–5% over each further rider's rides with only the mass data-implied — so the *energy law and the offset transfer across all three riders, while the ε geometry does not.* Because cumulative ascent is fractal and scale-dependent [Rapaport 2011], we fold a totals-only correction `k_smooth = 1 − c·x/h₊` (`c ≈ 3 m/km`) into the closed-form law to discount sub-metre noise without touching sustained climbs. We further derive an energy↔time dual, `x* = x + k₊·h₊ − k₋·h₋`, in which the descent coefficient `k₋` is the time-twin of `ε` through the same descent power; the *linkage* is novel (no prior work derives the descent time-credit from the same descent power as the energy recovery factor). Tested against measured moving time (§8.8), the ascent half transfers out-of-sample (6.6% median vs 7.6% naive on a second rider) while the descent bridge does not predict measured descent speed, so `k₋` stays an empirical coefficient. Both engines and the shared law are deployed in open, local-first tools (sampasimu, amora, quilojaules).
...
- **Assessment against a power-measured descent-energy-balance `ε`**: a 37% RMS reduction over the best flat constant on real descents (s̄ ≥ 3%, n = 22; in-sample, §8.3). Frozen and tested on two further independent riders (§8.6): the −0.13 offset recurs on both (gaps 0.12, 0.13), but the geometric ε *skill* is **rider-dependent** — it beats a coasting rider's own best constant by ~35%, and is inconclusive-to-failing for a fast descent-pedaller. The energy law and the offset transfer across all three riders; the ε geometry does not.
...
The net across three independent riders and meters: **the energy law and the calibrated −0.13 offset transfer robustly; the geometric ε *skill* does not — it wins for a coaster (P. Paz), and is inconclusive-to-failing for a fast descent-pedaller (JAAM).** That is exactly the paper's standing position (§8.3: ε's residual is *rider behaviour, not route geometry*), now demonstrated across riders rather than asserted. Full log: journal Entry 14.
...
**Three riders, three power meters — but geography and rider behaviour are the open limits.** The energy benchmarks come from three riders' own devices: the author (Datasets 1–2, 106 rides) and **two independent riders — P. Paz and JAAM, neither a Pedal Hidrográfico member** — (Datasets 4–5, 441 + 219 rides), whose data arrived after the calibrations were frozen. Two things temper the transfer. First, **geography is coincidentally, not structurally, controlled**: all three riders' power/descent benchmarks happen to fall in the São Paulo altitude band (~730 m) — JAAM's genuinely multi-country history is almost entirely in *non-power* activities, so its power + real-descent non-SP subset is n = 2 — meaning different climates, air densities, traffic regimes, and bike categories remain untested. Second, and more informative, **the geometric ε skill is rider-dependent** (§8.6): it transferred strongly to a coasting rider (P. Paz) but was inconclusive-to-failing for a fast descent-pedaller (JAAM), because the coasting-limit `ε_coast` only has signal when the rider actually coasts. The energy law and the −0.13 offset transfer across all three; the ε geometry does not. Riders 2–3's CdA/C_rr enter the energy scoreboard as *assumed* constants (only the mass is data-implied), but they are no longer untested — an independent per-activity power-balance fit corroborates them as plausible (the robustness note below; journal Entry 15), and JAAM's m̂ = 101.7 kg, once suspected an artifact, is rider-confirmed at ≈ 100 ± 7 kg (§8.6). The ε evaluation still shares its method (30 m cells, measured flat speed) across riders, so a method-level artifact would not be caught.

**The assumed rider physics is corroborated, not just posited.** Because Datasets 4–5 assume CdA and C_rr (only mass is data-implied), we tested whether those constants are defensible by recovering them from the rides themselves (journal Entry 15): a per-activity power-balance fit — virtual-elevation with a GPS-bearing wind vector, linearised to break a CdA↔wind degeneracy, mass from braking-free climbs — returns per-rider CdA, C_rr, mass and wind, and all three riders land inside plausible target ranges (P. Paz CdA ≈ 0.26 / C_rr ≈ 0.005; JAAM CdA ≈ 0.32 / C_rr ≈ 0.011). The method anchor-validates on the author, whose recovered CdA ≈ 0.33 (vs 0.39 assumed for his own rides) and C_rr ≈ 0.008 (vs 0.008 assumed) match the constants used to model them. This is a plausibility check, not a re-calibration — only ~25% of rides fit the single-rider balance cleanly (group/draft and urban stop-go rides break it), and absolute wind magnitude leans on a per-rider attenuation de-bias — and it changes no scoreboard number, which still uses the assumed constants. The recovered CdA does, however, run somewhat below the assumed 0.40 for both riders, the direction consistent with the small positive canonical bias (P. Paz +5.0%, §8.6) that an over-stated drag would produce.

**The ε correlations are in-sample and part–whole.** The −0.13 offset is calibrated on the same 44 rides the correlations are reported on, and the predictor `ε_coast` shares its dominant term *and* its per-ride α with the "truth" `ε_bal`, so the reported 0.77/0.82 are inflated by construction — the shared geometry term `α/(β·s̄)` alone correlates 0.72 with `ε_bal` and 0.99 with `ε_coast` itself (§8.3). Read them as a within-sample skill signal, not out-of-sample validation; the RMS-reduction figure (37% at s̄≥3%) is the more defensible statistic. The censo set supplies the cross-regime check, with a split verdict: the frozen −0.13 offset ties the in-sample-optimal constant on the descent-balance target (§8.5), so the *calibration* transfers — but the full geometric `ε_geom` under-performs a flat ε = 0.20 on the energy sweep (§8.4), so the clamp's gentle-terrain behaviour does not. Two further independent riders (§8.6) then supply the cross-rider check, with a nuanced verdict: the calibrated −0.13 offset recurs on both (measured gaps 0.12, 0.13), and the energy law transfers to both (~4–5% median), but the geometric ε *skill* is rider-dependent — it beats a coasting rider's (P. Paz) own best constant by ~35% on real descents, yet is statistically inconclusive for a fast descent-pedalling rider (JAAM) whose measured recovery is low. So the calibration and the energy law transfer across riders; the ε geometry transfers only when the rider actually coasts.
