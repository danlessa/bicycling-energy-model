...
- **Assessment against a power-measured descent-energy-balance `ε`**: a 37% RMS reduction over the best flat constant on real descents (s̄ ≥ 3%, n = 22), with correlations of 0.30–0.82 depending on weighting and subset (in-sample; §8.3 states the caveats and the worked example in full).
...
All power and GPS recordings analysed here were made by the author on the author's own device, including on group rides; no third-party physiological or location data is used or published. Activities recorded by other riders (16 of the censo links) were excluded rather than accessed. The repository history was audited for leaked personal data; raw tracks and spreadsheets are excluded from version control.
